...
- **有料の差別化**: 無料診断の範囲はそのまま含めたうえで、内部リンクの数など「ホームページの構成」に関わる指標を足し、機械的な判定だけで終わらせない。**回数制限つきの「パワーアップ分析」**にする。
...
| **ドメインパワー（新規）** | サイト全体の地力を 0〜100 で推定。内訳は外部からのリンクの評価（Open PageRank 0〜10。25 点）・ドメインの年数（RDAP の登録日。15 点）・インデックス数（`site:`。15 点）・対策キーワードの順位（15 点）・ブランド名検索の順位（10 点）・実ユーザーの規模（CrUX にデータがあるか。10 点）・サイトの規模（5 点）・信頼の手がかり（5 点）。取れなかった指標は分母から外し、配点 30 点分に届かなければ点を出さない。競合 2 件は Open PageRank と年数だけ並べる | Open PageRank（無料・要キー）+ RDAP（無料・キー不要）+ 既に集めた検索・CrUX・クロールの値 | 0 | ○ `src/lib/domain-power/` |
...
収集（30〜90 秒。進捗をストリーミング）
  ...
  ドメインパワー（RDAP の登録日 + Open PageRank。自社と競合 2 件）
...
| F′ | ドメインパワー（`src/lib/domain-power/`。`rdap.ts` = 登録日、`openpagerank.ts` = 外部リンクの評価、`score.ts` = 8 指標の配点、`collect.ts` = 自社と競合の取得）。報告書の KPI とカード、事実シートの領域 `domain`（ID は `D-01`〜） | **実装済み（2026-09-15）** |
...
| # | 決めること | 選択肢と推奨 |
|---|---|---|
| 1 | パワーアップ分析の回数 | **推奨: オールインワンで月 10 回**（1 サイトを月 1〜2 回 + 施策後の確認）。追加は当面なし。運営者（ADMIN）は無制限 |
...
| 6 | 画面の置き方 | **推奨: SEO タブに「パワーアップ分析」を 1 つ追加**（`/tools/seo-analysis`。入力 → 進捗 → 報告書 → 履歴）。既存のサイト診断・ページ最適化レポートはそのまま残す → **09-15 の利用者の指示でサイト診断はパワーアップ分析に統合**（同じクロールを二重に持たない。課題一覧・ページ一覧・CSV は報告書の「詳細」に）。ページ診断（キーワード × 競合上位 10 件）は軸が違うので別のまま |
