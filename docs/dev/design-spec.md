...
| 4 | ページ × カテゴリ 一覧 | 導入文「同じサイトでもテンプレートと個別コンテンツの違いでページごとに点数が変わります。」→ HeatTable。行 = 総合の低い順（入力 URL は先頭固定）。脚注「セルの色: 80 以上 = 合格域 / 50〜79 = 改善域 / 50 未満 = 未対応域」 | HeatTable |
...
| 付録 A | 診断ページ一覧 | `@md:grid-cols-2` の `dl > div`（BREAK_SELECTOR 対象）: # / パス / 総合 / グレード。12px、行高 26px（300 ページで約 3,900px）。続いて小表「診断できなかったページ」（URL / 理由）と「ページ収集方法: sitemap.xml（542 件中 128 件を診断、上限 SITE_MAX_PAGES）」 | — |
| 付録 B / 次のステップ | 3.2 と同じ | — |
