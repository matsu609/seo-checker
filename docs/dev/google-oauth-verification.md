...
| サイト診断・精密診断・AI 検索モニタリング・FAQ 提案 | MEO 採点の 9 項目の自動取得（いまはオーナー申告で代替済み） |
...
**→ 2026-09-21 「オーナー確認を行う」→「次へ」の次の画面: 「確認のため、送付先住所を入力してください」「こちらに入力していただいた住所は公開されません。私書箱は対象外です」（国 / 郵便番号 / 都道府県 / …）。利用者の質問「送付先住所とは？」**

| 論点 | 答え |
|---|---|
| これは何か | プロフィールが「拠点なし」で住所を持たないため、**確認の手続きの中でだけ Google が使う実在の住所**。公開されず、プロフィールにも載らない。動画の撮影場所と突き合わせるための基準（ハガキ確認のときは送付先にもなる） |
| 何を入れるか | **シェアオフィスの住所**（建物名・9 階・部屋番号まで）。**登記の住所（代表の自宅）は入れない**（動画はシェアオフィスで撮るので、ここがずれると要件 1 で落ちる） |
| 入れてはいけないもの | 私書箱・郵便受けだけのバーチャルオフィス（画面に「私書箱は対象外」と明記） |
| 先の説明との整合 | §6-7 で「住所はどこにも入れなくてよい」と書いたのは**プロフィール**の話。**確認の手続きでは聞かれる**。ここで入れた住所はプロフィールの「拠点なし」を変えない |

**→ 2026-09-21 利用者の質問「社内システムやオンラインビジネスツールの操作は、Google Workspace だけではダメか」**

| 論点 | 答え |
|---|---|
| Workspace は「オンラインビジネスツール」に当たるか | **当たる。**Google の例示は「業務用機器 / 予約システムの外観 / 社名入り車両」、ヘルプは「POS システムへのアクセス」「従業員だけが使えるもの・エリア」「ビジネス名の載った書類」。`@wolf-info.org` の Workspace は**会社のドメインで会社が発行した業務ツール**なので、予約システム・POS と同じ種類。しかもドメインがサイト `wolf-info.org` と一致するので、汎用の予約システムより結びつきは強い |
| Workspace **だけ**で通るか | **通る可能性はあるが、保証はできない**（審査は人がチェックリストで見る。Gmail の画面は一見すると汎用に見える）。**紙の書類が無くても、次の 3 点セットにすれば十分**: ①キーカードで入室（従業員しか入れないエリア）②名刺（自分の名前 + 株式会社Wolf）③Workspace。①②は撮影順にすでに入っているので、**追加で用意するものは無い** |
| Workspace の見せ方（ここで強さが決まる） | (a) Gmail: 画面上部のアカウントを開いて **`…@wolf-info.org` と自分の名前**を映す（3 秒）。(b) **Google カレンダー**: イベント管理会社なので**案件・現場の予定**が入っているカレンダーが「予約システム」そのもの。(c) **Google ドライブ**: 「株式会社Wolf」のフォルダを開き、**見積書・請求書・契約書の PDF を 1 つ開く** → 画面上で社名が読める = **「ビジネス名の載った書類」も同時に満たす**（紙が無くてよい）。(d) 管理コンソール（admin.google.com）に入れるなら組織名「株式会社Wolf」が出る画面が最強 |
| やってはいけないこと | 顧客の個人情報・金額の細部・口座番号をアップで映さない（社名と書類の種類が読めれば十分）。他社のメールは映さない |

**→ 結論: Workspace「だけ」に頼らず、キーカード入室 + 名刺 + Workspace（Gmail のアカウント表示 → カレンダー → ドライブで社名入り PDF）の順で見せれば、紙の書類が無くても要件 3 は満たせる。**
