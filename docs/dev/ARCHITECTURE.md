...
| マスターアカウント用 | `/admin` | マスター画面（**システム側だけ**: 版・外部連携の設定状況・定期処理・管理アカウントの追加と解除）。`ADMIN_EMAILS` の人だけ。ほかは 404 | — | Clerk |
| マスターアカウント用 | `/admin/feedback` | お客様からのご意見・不具合の一覧と返答（**運用者だけ**。2026-09-21 に顧客管理から分離） | — | Supabase（`feedback`） |
...
- サイドバーの管理系タブは 2 つ（2026-09-21）。**管理者用**（顧客管理・デモ用の無料クイック診断。運用者と管理アカウント）と**マスターアカウント用**（マスター画面・ご意見・不具合。運用者だけ）。
