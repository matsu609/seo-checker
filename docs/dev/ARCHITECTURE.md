...
| 管理者用 | `/clients` | 顧客管理（契約状況・ご利用状況・ご意見への返答・割引・機能の個別開放・代理ログイン）。運用者は全登録者 + 担当の割り当て、管理アカウントは担当の登録者だけ。ほかは 404 | — | Clerk / Supabase（ご意見） |
| 管理者用 | `/admin` | マスター画面（**システム側だけ**: 版・外部連携の設定状況・定期処理・管理アカウントの追加と解除）。`ADMIN_EMAILS` の人だけ。ほかは 404 | — | Clerk |
...
| 役割 | 決まり方 | 見えるもの | できること |
|---|---|---|---|
| マスター（運用者） | 環境変数 `ADMIN_EMAILS` に書いた**確認済み**のメール | `/admin`（システム側）と `/clients` で**登録しているすべてのお客様**。**代理ログイン**でお客様の画面そのものも | 管理アカウントの追加・解除、担当の割り当て、ご意見への返答、割引、機能の個別開放。ツールは全部使える |
| 管理アカウント | `publicMetadata.role === "agency"` | `/clients` で**自分に割り当てられた登録者だけ**。**マスター画面（`/admin`）は見えない** | ご意見への返答、割引、機能の個別開放、代理ログイン（担当の登録者だけ）。**ツールはカードの登録なしで全部使える**（2026-09-20） |
| 登録者 | 上のどちらでもない | 自分のツール画面だけ | — |

- **マスターだけ環境変数**にしてある。運用者の権限を Clerk の値に置くと、Clerk に入れた誰かが自分を運用者にできてしまう。Vercel の設定を触れる人だけが変えられる場所に置く。
- 線引きは「システムが見えるかどうか」（利用者の指示 2026-09-20）。版・外部連携・定期処理はマスターだけ、お客様の情報は両方。管理アカウントは自分の画面だけでお問い合わせに答えきれる。
- 管理アカウントの「担当」は登録者側の `publicMetadata.agencyId`（管理アカウントの Clerk ユーザー ID）。**割り当てはマスターだけ**が行う（`/clients` の顧客一覧）。
- 顧客 1 人への操作は必ず `requireClientAccess()`（`src/lib/admin/guard.ts`）を通す。運用者は全員、管理アカウントは担当の登録者だけで、担当外・他の管理アカウント宛は 404。判定に使う ID は必ずログイン中のセッションから取る（`currentAgencyId()`）。リクエストで受け取った ID を信用すると他の管理アカウントの担当が見えてしまう。
- ツールの開放は 3 か所が同じ判定でそろっている: `checkPlanForFeature`（ログイン中）・`canUseFeature`（画面の鍵表示）・`accessAllows`（定期処理）。運用者・管理アカウントはどれも全機能。
