...
| 82 | **GSC / GA4 / CRM の自動診断（約 120 ルール）+ コンサル回答生成**: 利用者から 2026-09-15 に仕様書を受領。正本は [diagnosis-rules-spec.md](./diagnosis-rules-spec.md)（§1〜§25 が仕様、§26 が現状との差分、§27 が実装の段階 G1〜G8、§28 が決めてもらうこと 8 件）。**ルールは 1 件も未実装**（既存の 48 ルールはクロールした HTML のテクニカル SEO で別物）。LLM 側（事実シート・構造化出力・数値の照合・セカンドオピニオン）はパワーアップ分析のものを流用できる。着手前に §28 の 1（入口: 既存の Google 連携か CSV か）と 2（パワーアップ分析とは別ツールにするか）を決める必要がある | 利用者 → Claude | **仕様の取り込みのみ完了（09-15）。§28 の判断待ち** |
...
- 自動診断（#82）の §28: ① **入口**を既存の Google 連携（OAuth）にするか、GSC / GA4 の CSV アップロードにするか（仕様書はフェーズ 1 を CSV 前提で書いているが、このアプリには連携が既にある）② **パワーアップ分析とは別ツール**（仮称「月次診断」）にするか、1 つに統合するか③ **初期リリースの範囲**（推奨: データ品質 15 件 + GSC 約 60 件 + LLM をまず通し、GA4 は次）④ severity を 4 段で独立させるか ⑤ 人間による確認の工程を最初から入れるか ⑥ CRM は保留でよいか ⑦ 閾値は定数 + 業種プリセットでよいか ⑧ 回数制限は別枠にするか
