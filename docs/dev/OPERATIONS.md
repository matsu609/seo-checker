...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r28 | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| 46 | **検索順位の計測（候補、提案中）**: 店舗ごとに「地域 + 業種」のキーワードを最大 5 本登録 → 週 1 回の一斉更新で Text Search（`locationBias` = 店舗の座標）を叩き、自社が何位に出るかを記録・推移グラフ・競合の順位も同時に取れる。費用は Text Search Pro 区分（月 5,000 回まで無料。1 店舗 5 KW × 月 4 回 = 20 回 → 250 店舗まで無料枠内）。実装 1 日 | Claude | 利用者の GO 待ち |
| 47 | **周辺の同業との相対位置（候補、提案中）**: Nearby Search（店舗の座標から半径 1〜2km、同じ primaryType）で周辺 20 店舗の評価・件数・写真数を取り、「地域内で評価は上位 何%、件数は 何位」を報告書に出す。競合を登録していなくても比較できる。費用は Nearby Search Pro 区分（月 5,000 回まで無料。週 1 回 × 店舗数）。実装 半日 | Claude | 利用者の GO 待ち |
