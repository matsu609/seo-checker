...
| `GOOGLE_PLACES_API_KEY` | **設定済み**（09-10 15:58、Secret、Production） | seo-checker の Places API (New) 制限つきキー |
...
| 23 | 「AI に相談する用のテキストをコピー」ボタン（候補）: `toCommentaryInput` の JSON と `SYSTEM_PROMPT` 相当の指示文を 1 つのテキストにしてクリップボードへ。運営者が自分の Claude Pro に貼って総評を作る用途。API キー不要 | Claude | 利用者の判断待ち（コンサル型なら優先） |
