...
利用者「手順が多くて顧客にやらせるには無理があるかもしれない。SNS の連携だったりとか AIO 対策として他にできることは何があるか」。**コード変更なし。方針の整理だけ。**

#### ① 指摘は正しい。r126 の掲載タブは 09-17 の原則に反している

09-17 の決定「**お客様側の作業が要る機能はそもそも置かない**」（llms.txt を外した理由）に照らすと、掲載タブの 30 媒体のうち `manual` 13・`monitor` 14 は顧客の画面に置くべきものではない。llms.txt を外したのと同じ理由で、ここも整理が要る。

**解き方は 3 つしかない（機能追加では解けない）。**

| 道 | 中身 | 顧客の作業 | 我々のコスト | 判断 |
|---|---|---|---|---|
| A. 絞る | 顧客の画面には日本で効く 8 媒体だけ出す。残り 22 は畳む | 30 分 × 1 回 | ゼロ | **すぐやる** |
| B. 代行にする | `manual` の作業を `/agency`・`/admin` 側の作業画面に移す。顧客は結果だけ見る | ゼロ（委任の記録 1 回） | 初回 2〜3 時間 / 社 | **有料オプションにする** |
| C. 買う | Uberall 等の OEM で Apple / HERE / TomTom / Waze へ配信 | ゼロ | 月額（未照会） | 照会の答え待ち |

**顧客に出す 8 媒体（案）**: Google（api）/ Yahoo!プレイス（file）/ Bing Places（file）/ Apple Business Connect / i タウンページ / エキテン / Facebook・Instagram / 業種別 1 つ。

#### ② SNS 連携 — 効くもの / 効かないもの

**効かない（やらない）**: いいね・シェア・フォロワー数は検索順位の要因ではない（Google が繰り返し明言）。毎日投稿すること自体も SEO には効かない。

**効く（AIO の文脈で）**:

1. **プロフィールの存在と NAP の一致** — AI が「実在する事業者」と判断する材料。`sameAs` で公式サイトと結ぶ。**1 回作れば終わり**（精密診断はすでに `sameAs` の有無を見ている → `src/lib/analyzer/jsonld.ts`）
2. **Google ビジネス プロフィールの投稿（最有力）** — MEO の評価に直接効く。**`mybusiness.googleapis.com/v4` の `localPosts` は口コミ返信と同じ `business.manage` スコープ**なので、申請中の #5 が通れば**追加申請なしで投稿できる**。未実装（`src/lib/google/business-profile.ts` は accounts / locations / reviews / NAP 更新だけ）
3. **1 回書いて多媒体に同時配信** — AI ライティングで原稿 → GBP + Facebook + Instagram。顧客の作業は初回の接続だけ

**API の現実**: X は Basic で月 $200（中小には重い）。Meta（Facebook / Instagram）は App Review + ビジネス認証が要る。**GBP だけが追加コストゼロで動かせる。**

#### ③ AIO として他にできること（効き ÷ 顧客の作業、の良い順）

| # | 施策 | 顧客の作業 | 我々の作業 | 効き方 |
|---|---|---|---|---|
| 1 | **掲載の生存監視**（`evidence_url` / `last_checked_at`）| ゼロ | 自動 | 「登録した」ではなく「**今も正しく出ている**」の証明。r126 の残タスク 1 番。商品価値そのもの |
| 2 | **GBP 投稿の自動配信** | 初回の接続のみ | 原稿生成 | MEO の評価 + AI が拾う「更新されている事実」 |
| 3 | **プレスリリース配信の代筆**（PR TIMES 等）| 承認のみ | 執筆 | AI 検索が最も拾いやすい第三者メディア。転載で一気に面が増える。1 本 3 万円前後（**要確認**）|
| 4 | **構造化データ（Organization / LocalBusiness / FAQ / sameAs）の生成** | 設置（or 我々が請ける）| JSON-LD 生成 | いまは精密診断が「無い」と言うだけで**直す手段が無い**。ここが最大の穴 |
| 5 | **Wikidata への登録** | ゼロ | 30 分 | AI とナレッジパネルの裏づけ。Wikipedia より緩いが**出典が無いと消される**（法人番号・公式サイト・掲載媒体が揃っていれば残りやすい）|
| 6 | **法人番号公表サイト / gBizINFO** | ゼロ | 確認のみ | 国の一次情報。`sameAs` に入れられる。登記済みなら既に存在する |
| 7 | **求人媒体の会社ページ**（Indeed / Wantedly / 求人ボックス）| ゼロ〜少 | 入力 | 無料・NAP が載る・被リンクにもなる |
| 8 | **業界団体・商工会議所の会員名簿** | 入会判断 | 申請補助 | 09-19 の被リンクの段 1 と同じ。質が高い |
| 9 | YouTube | 撮影 | 編集 | AI Overviews / Gemini が強く引用するが**制作コストが高い**。優先度は下 |

**やらないこと（法令リスク）**: 口コミ依頼に対価を渡す、Yahoo!知恵袋・note での自作自演 → **ステマ規制（景品表示法の指定告示、2023-10-01 施行）違反**。有料リンク・相互リンク集は Google のスパムポリシー違反。

#### 実装の優先順（提案・利用者の判断待ち）

1. **掲載タブの絞り込み**（上記 A）— 半日。原則違反の解消が先
2. **GBP 投稿の配信**（`localPosts`）— 1 日。#5 の承認が下りたら即戦力
3. **掲載の生存監視**（`evidence_url` / `last_checked_at` / `next_check_at`）— 1〜2 日
4. **構造化データ生成**（HP 改修提案の中に「貼るだけの JSON-LD」を出す）— 1 日

**注記**: この環境は外向きのページ取得がプロキシで塞がれているため、料金・API の条件は記憶に基づく。**発注前に各社の一次情報で必ず確認すること。**

### 2026-09-19（掲載を 7 媒体に絞り、ビジネス プロフィールへの投稿を追加、r127）

利用者の指示:「1 と 2 の順でお願いします」（= ① 掲載タブの絞り込み ② GBP 投稿の配信）。

#### ① 掲載タブの絞り込み

09-17 の原則「お客様側の作業が要る機能はそもそも置かない」に照らすと、r126 の掲載タブは 30 媒体のうち **27 が手作業**で原則違反だった。llms.txt を外したのと同じ理由で整理した。

| # | 内容 | 触ったところ |
|---|---|---|
| 1 | 媒体マスタに **`tier`（`core` / `advanced`）** を追加。`tier` を書いていない媒体は `advanced`（畳む側） | `src/lib/listings/media.ts` |
| 2 | 日本で効く **2 媒体を追加**: i タウンページ、エキテン | 同上 |
| 3 | `publishTargets` の**既定を core に**。上級は `scope: "all"` か媒体の名指しのときだけ | `src/lib/listings/publish.ts` |
| 4 | `POST /api/listings/publish` に `scope`（既定 `core`）を追加 | `src/app/api/listings/publish/route.ts` |
| 5 | 媒体一覧は core を素の一覧に、上級 25 は `details` に畳む。集計・ボタンの件数・冒頭の説明も core が既定 | `src/components/listings/ListingsTool.tsx` |
| 6 | 「どこに載っているか調べる」で i タウンページとエキテンを媒体に紐づけ、主要媒体の掲載状況に数える | `src/lib/citations/sources.ts` |

**お客様に出す 7 媒体**（`CORE_MEDIA_IDS`）: Google マップ / Yahoo!プレイス / Bing Places / Apple Business Connect / i タウンページ / エキテン / Facebook・Instagram。

**テストで固定したこと**（次のセッションが崩さないように）:
- `tier: "core"` と `CORE_MEDIA_IDS` は**両方直さないと落ちる**（片方だけの変更を防ぐ）
- core の媒体は**すべて `kind: "self"` かつ `integration !== "monitor"`**（「登録できません」だけの行がお客様の画面に並ばないように）

#### ② ビジネス プロフィールへの投稿（`/tools/posts`）

**いちばん大事な点: `localPosts` は口コミ返信と同じ `business.manage` スコープ**なので、**追加の申請は要らない**。申請中の #5（ケース ID `0-4126000041187`）が承認されれば、コードの変更なしで動き出す。

| # | 内容 | 触ったところ |
|---|---|---|
| 1 | My Business v4 の `localPosts`（一覧・投稿・削除）と本文の組み立て | `src/lib/google/business-profile.ts` |
| 2 | 画面とサーバーで共有する定数（本文 1,500 字・ボタン 6 種・状態のラベル） | `src/lib/posts/constants.ts`（新） |
| 3 | AI の下書き（1 行目に用件。URL・電話番号・ハッシュタグ・絵文字・最上級は書かせない。口コミは区切りブロックに入れて渡す） | `src/lib/posts/draft.ts`（新） |
| 4 | `GET/POST/DELETE /api/posts`、`POST /api/posts/draft`、`GET /api/posts/status` | `src/app/api/posts/`（新） |
| 5 | 画面: 接続 → ネタを 1 行 → AI 下書き → 編集 → ボタンを選んで投稿 → これまでの投稿と削除 | `src/components/posts/PostsTool.tsx`（新） |
| 6 | サイドバーの MEO に「投稿（最新情報）」を追加（スタンダード） | `src/lib/features/registry.ts`、`src/components/shell/icons.tsx` |

**決めた線**:
- **空の項目は送らない。**`CALL`（今すぐ電話）には URL を付けない（Google が弾く）。URL が空のボタンは付けない。
- 投稿は **Google の審査を通ってから掲載**される（数分〜数時間）。画面は `LIVE` / `PROCESSING` / `REJECTED` をそのまま出す。
- 投稿はこのアプリに**保存しない**（Google が持つ）。ブラウザに覚えるのは「最後に選んだビジネス・ボタン・リンク先」だけ。

**未対応（次の手）**: イベント・特典の投稿（日程の入力が要る）、写真の直接アップロード（いまは公開 URL の指定）、Instagram / Facebook への同時配信（Meta の App Review が要る）、MEO 診断の「投稿」項目からこの画面へのリンク。

#### 検証

`npx eslint`（0）・`npx tsc --noEmit`（0）・`npx vitest run`（141 ファイル / 1,658 件すべて成功）・`npx next build`（成功。`/tools/posts` と `/api/posts` が出ている）。

#### 本番で動くか

- **掲載の絞り込みは今すぐ効く**（Google 以外の媒体はもともと API を使わない）。
- **投稿は #5 の承認待ち**。承認前は 403 になり、「Business Profile API の利用申請が承認され、3 つの API が有効になっているか…」の案内が出る。

### 2026-09-19（本番の画面を見ての修正 2 件、r128）

利用者から本番（`/tools/citations`）のスクリーンショットを受領。**このとき本番は r126**（main が未マージのため r127 は入っていない）。画面から 2 件見つけて直した。

#### ① 機能の説明文が「30 媒体」のままだった（r127 の漏れ）

媒体マスタと画面は 7 媒体に絞ったのに、**機能の説明文（`registry.ts` の `description` / `details`）と料金プランの売り文句を直し忘れていた**。PageHeader・料金表・準備中ページはここを引くので、お客様には「30 媒体」と見えていた。

| 直したところ | 前 | 後 |
|---|---|---|
| `registry.ts` citations の説明 | 「そのまま 30 の地図・検索・ディレクトリへ」 | 「日本で効く 7 媒体へ」 |
| 同・**被リンクの説明** | 「**被リンクを増やす打ち手そのものです**」 | 「多くの媒体は nofollow なので被リンクへの効果は限定的。効くのは指名検索の受け皿・NAP の一貫性・AI が拾う事実の裏づけ」（09-19 の結論に合わせた） |
| `registry.ts` citations の details | 「30 の媒体に同じ内容で登録」 | 7 媒体を名前で列挙 + 上級は畳んである旨 |
| `registry.ts` listings の説明・details | 「30 の地図・検索・ディレクトリ」「無料で登録できる媒体（… 9 媒体を列挙）」 | 7 媒体を名前で列挙 |
| `plans/catalog.ts` スタンダードの売り文句 | 「30 媒体への一括掲載（NAP）と **llms.txt 生成**」 | 「日本で効く 7 媒体への一括掲載（NAP）」。**llms.txt は r126 でサイドバーから外したので売り文句からも外した**。MEO の行に「投稿（最新情報）」を追加 |

**次のセッションへの注意**: 媒体の数や範囲を変えるときは `media.ts` だけでなく **`registry.ts` の description / details** と **`plans/catalog.ts` の highlights** も必ず直す（テストでは縛れていない自然文なので、grep して確認する）。

#### ② タブ列に縦スクロールバー（▲▼）が出ていた

スクリーンショットのタブ列（「どこに載っているか調べる／掲載先に登録する」）の右端に ▲▼ が出ていた。

- **原因**: `src/components/ui/Tabs.tsx` の `overflow-x-auto`。CSS の規定で、片方が `visible` でもう片方がそうでないときは `visible` が `auto` になるため、**`overflow-y` も `auto` になる**。各タブの `-mb-px`（下線を親の罫線に重ねるための負のマージン）が 1px のはみ出しになり、**Windows の Chrome（オーバーレイでない従来型スクロールバー）でだけ**縦スクロールバーが描かれていた。Linux のヘッドレスはオーバーレイなので開発中は見えない。
- **確認**: Playwright で `clientHeight 39 / scrollHeight 40`、`overflow-y: auto` を実測。
- **修正**: `overflow-y-hidden` を足した（`overflow-y: hidden` を実測で確認）。**タブ下線の描画は修正の前後でピクセル単位（4 倍スケール）で同一**であることを確認済み。`Tabs` は全画面共通なので、ほかのタブ列でも同じ ▲▼ が消える。
- 同じ `overflow-x-auto` を使う他の箇所（DataTable・表のラッパー）は負のマージンを持たないので影響なし。

#### 検証

`npx eslint`（0）・`npx tsc --noEmit`（0）・`npx vitest run`（141 ファイル / 1,658 件すべて成功）・`npx next build`（成功）。

#### 本番の状態（重要）

**この時点の本番は r126。**r127（7 媒体への絞り込み・投稿）と r128（この修正）は**ブランチにあるだけで main に未マージ**。サイドバーの「MEO 2」（投稿が無い）と「30 の地図・検索・ディレクトリ」の文言が、未マージであることの目印。

### 2026-09-19（相談: Wikidata・法人番号公表サイト / gBizINFO は簡単か）

利用者の質問に対する整理。**コード変更なし。**

#### 結論: 2 つは性質がまったく違う

| 施策 | 登録作業 | 我々の作業 | 自動化 | 効果 |
|---|---|---|---|---|
| **法人番号公表サイト**（国税庁） | **不要**。登記すると国税庁が 13 桁の法人番号を指定し自動で載る | URL を控えて `sameAs` に入れるだけ（5 分） | ◎ | 小〜中（`sameAs` の裏づけ、AI の事実確認） |
| **gBizINFO**（経産省） | **不要**。法人番号があれば自動でページがある | 同上（5 分） | ◎ | 小 |
| **Wikidata** | 項目作成・プロパティ入力・出典付け | **30 分〜1 時間 / 社**。**自動化は規約違反**（ボット編集は事前承認制） | ✗ | **不確実**。削除依頼で消えることが実際にある |

#### 大きな落とし穴: 個人事業主には法人番号が無い

**御社（SEO 研究所・個人事業）自身がこれに当たる。**店舗のお客様も個人事業が多いので、**使えるのは法人登記しているお客様だけ**。導入前に必ず確認する。

#### Wikidata の扱い（09-19 の前の回で「Wikipedia より緩い」と書いた点の補正）

「緩い」＝「簡単」ではない。**特筆性（notability）のポリシーがあり**、「公開された信頼できる出典で記述できる実体」が要件。中小企業の項目は宣伝目的と見なされると削除される。**ボット編集は事前承認制**なので機械化もできない。→ **商品メニューには入れない。**やるなら「法人番号・公式サイト・掲載媒体が揃ったお客様に、我々が手作業で」＝ 代行メニューの一項目に留める。

#### 本当に価値があるのは法人番号 Web-API（提案）

**国税庁の法人番号システムには無料の Web-API がある。**会社名から法人番号を引くと**登記上の正式な商号と本店所在地**が返る。これが効く:

1. **NAP の正本を国の一次情報で検証できる。**「株式会社○○」と「(株)○○」の表記ゆれを、推測でなく登記で確定できる。いまの掲載タブは Google マップの公開情報とだけ突き合わせているが、**Google より信頼できる出典**になる
2. **`sameAs` の URL を自動生成できる。**法人番号公表サイトと gBizINFO の URL は法人番号から機械的に作れる → 残タスク 4（構造化データ生成）の中身がこれで埋まる
3. **お客様の作業はゼロ。**会社名を入れるだけ

**実装の見積り: 半日。**残タスクの 3（生存監視）・4（構造化データ）より先にやると、4 の材料が揃うぶん効率がよい。

**要確認（この環境は外部ページを取得できないため記憶に基づく。発注・実装前に一次情報で確認すること）**: アプリケーション ID の申請方法・発行までの日数・利用条件、API の現行バージョン、gBizINFO 側の API トークンの要否。
