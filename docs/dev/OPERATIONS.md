...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r51 | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| 72 | **MEO 報告書に「優先改善リスト」と「採点基準の付録」を足す（B・C）**: 配点 × 現状で「直すと +N 点」を出す / 21・28 項目の配点と判定条件を開示。クイック診断と精密診断の両方に出す。未取得 9 項目は「オーナーにしか分からない項目」としてまとめ、精密診断への導線にする | Claude | 着手 |
| 73 | **口コミの傾向分析（D。精密診断のみ）**: 履歴に貯まった口コミを重複排除して集計し、頻出語・良い点 / 不満の兆候・星別の傾向を出す。生成 AI 不使用・API 費用ゼロ | Claude | 未 |
| 74 | **NAP 整合チェック（E。精密診断のみ）**: 登録サイトを自前クローラで読み、JSON-LD と本文から店名・住所・電話を抜いて Google の値と突き合わせる（`src/lib/listings/profile.ts` の `compareNap` / `normalizeForCompare` を再利用）。追加の API 費用はゼロ | Claude | 未 |
...
クイック診断の 2 本（r49）は、専用のヘッダー（ロゴ・ログイン・「初月無料ではじめる」・規約）だけの画面で開く。サイドバーも有料ツールの一覧も出ないので、**そのまま見込み客に渡して構わない**。診断の結果の下に「クイック診断で分かるのは『今の状態』まで」→「精密診断で分かること」→「初月無料で精密診断をはじめる」（`/sign-up`）が必ず出る。管理画面のサイドバーでは最下部の「お客様に渡すクイック診断」から開ける。
