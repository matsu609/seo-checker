...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r105 | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| 1c | **割引コードを 2 種類作る（r105 から初月無料も自動ではなくコード。相手によって使い分ける）** | https://dashboard.stripe.com/test/coupons | 下の「割引コード（クーポン）の運用」のとおり。①月額の値引き（例: 金額割引 ¥20,000・期間「永続」）②初月無料（**割引率 100%・期間「1 回」**）。それぞれ「プロモーションコードを追加」で顧客に渡すコードを作る。コードを持たない人は定価のまま（定価表示が二重価格表示にならない） |
...
全員向けの無料期間は無い。値引きしたい相手にだけ、Stripe で作ったプロモーションコードを渡す。相手は申し込み画面（Stripe Checkout）の「プロモーションコードを追加」に入力する。コード無しの人は定価。

| 渡したいもの | クーポンの作り方（Stripe → 商品カタログ → クーポン →「新規」） | 相手の請求 |
|---|---|---|
...
- 作る場所: テスト https://dashboard.stripe.com/test/coupons ／ 本番 https://dashboard.stripe.com/coupons （本番モードで作り直す。テストのクーポンは本番で使えない）
...
| 5 | 割引コードの発行（#58 の 1c。月額の値引きと初月無料の 2 種類。r105 から初月無料も自動ではない） | 利用者 |
