...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r132（別セッションの棒グラフまで）。**r133（管理アカウントの全機能開放・顧客管理の切り出し）は作業ブランチ `claude/vigilant-gates-vxlipt` に push 済みで、main へのマージ待ち** | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合）。**マージするまで r133 は本番に出ない** |
...
| 権限の線引き（r133、09-20） | **コード完成・検証済み（lint / tsc / test 1,808 件 / build）。本番確認は未（#126）。main へのマージも未** | 利用者の指示 09-20。**管理アカウントはカードの登録なしでツールを全部使える**（運用者と同じ扱い。判定は `checkPlanForFeature` / `canUseFeature` / `accessAllows` の 3 か所）。マスターとの違いは 2 つだけ: ①マスター画面（`/admin` = 版・外部連携・定期処理・管理アカウントの追加）が見えない ②見えるお客様は担当分だけ。お客様の契約状況・ご利用状況・ご意見への返答は**顧客管理（`/clients`）**に切り出し、サイドバーのタブ名は「運用」→「**管理者用**」。無料診断のデモ枠（月 50 回）は据え置き |
...
| 126 | **r133 を main にマージして本番に出す**（作業ブランチ `claude/vigilant-gates-vxlipt`）。マージしないと管理者用タブ・顧客管理・管理アカウントの全機能開放は本番に出ない。マージ後に `/clients` と `/admin` を開いて #94 の確認へ | 利用者（または次のセッション） | 未 |
...
**注意**: この作業は**ブランチ `claude/vigilant-gates-vxlipt`**にあり、main には入っていない（#126）。同じ日に別セッションが main へ r132（棒グラフ）を入れたので、こちらの版番号は **r133** に振り直してある。main へマージするまで本番には出ない。
