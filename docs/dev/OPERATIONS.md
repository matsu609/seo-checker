...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r141（ご意見の通知バッジ・管理アカウントのページ。09-21） | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| 権限の線引き（r133・r137〜r141） | **main にマージ済み・本番に反映（09-21 に利用者が `/admin` で確認。サイドバーが「管理者用」になり、管理アカウントを 1 件追加できた）。残りは管理アカウント側から見たときの確認（#94）** | 利用者の指示 09-20。**管理アカウントはカードの登録なしでツールを全部使える**（運用者と同じ扱い。判定は `checkPlanForFeature` / `canUseFeature` / `accessAllows` の 3 か所）。マスターとの違いは 2 つだけ: ①マスター画面（`/admin` = 版・外部連携・定期処理・管理アカウントの追加）が見えない ②見えるお客様は担当分だけ。お客様の契約状況・ご利用状況・ご意見への返答は**顧客管理（`/clients`）**に切り出し、サイドバーのタブ名は「運用」→「**管理者用**」。無料診断のデモ枠（月 50 回）は据え置き |
...
- **Wolf の確認（#5 / #54 ①。09-21 に利用者が着手）の結果。**09-21 のスクリーンショットで**「確認が必要」の状態と確定**（§7-1 の 0-1 (b)）。**利用者は動画を撮れない（09-21）。**①「オーナー確認を行う」を押して**出てきた方法の一覧**（電話 / メール / ハガキ / 動画 / ライブビデオ）を教えてもらう ②動画しか無ければ Wolf 側（`wolf@wolf-info.org`）に依頼できるか ③翠煙をどの Google アカウントで登録したか（確認済みのプロフィールなら誰も撮らなくてよい）。手順は [google-oauth-verification.md](./google-oauth-verification.md) §7-1「0-2 の補足」。①確認が完了したか（動画確認なら審査 3〜5 営業日） ②完了後に送る再申請の**新しいケース ID** ③却下された場合はその理由（文面ごと共有してもらう）。**確認完了日が元の日付のままか、再確認の日にリセットされるかは Google 側にしか分からない**ので、推測せず申請して結果を見る
