...
| 114 | **口コミ返信をツール内で完結させるための「API 以外」の作業**（2026-09-17 の調べ。詳細は作業ログ）: ① プライバシーポリシー第 5 条「書き換え・投稿の権限は要求しません」を **`business.manage`（書き込み）を要求する**内容に書き換え、口コミ本文を Anthropic に送ることと「返信は必ず利用者が確認して投稿する」を明記（審査の地雷 #7） ② 用途説明文とデモ動画（同意画面 → 口コミ一覧 → AI 返信案 → 投稿、を通しで） ③ Clerk のアプリ名 `My Application` を `SEO Checker` に（#7）、ブランディングに規約 / ポリシーの URL（#8） ④ 承認後: Google My Business API（v4）の有効化 → `/tools/replies` で接続 → 自分のプロフィールで投稿まで通す ⑤ 任意: 新着口コミの通知（My Business Notifications API + Cloud Pub/Sub）と Performance API（#113） | Claude（①②の文案）→ 利用者（③④・撮影・申請） | 未 |
