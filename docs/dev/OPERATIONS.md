...
| 41 | 料金: ~~オールインワン 9,800 円（r26）~~ → ~~定価 50,000 円の 1 本 + クーポン割引（r46）~~ → **3 段階に決定（r63、利用者の決定 2026-09-15）**: ライト 38,000 円（診断・計測）／**スタンダード 50,000 円（本命）**（+ AI が改修案・原稿を作成）／プレミアム（伴走）150,000 円（+ 人の作業。月 3 社まで・問い合わせ受付）。実装・文面・紹介サイトまで完了 | 利用者 → Claude | **完了（r63）**。残りは Stripe 側の作業（#58 の 1 と 3）だけ |
...
| 82 | **GSC / GA4 / CRM の自動診断 + コンサル回答生成**: 仕様は [diagnosis-rules-spec.md](./diagnosis-rules-spec.md)。利用者の決定（09-15）= **入口は既存の Google 連携（OAuth）のみ・CSV は作らない／パワーアップ分析と同義（別ツールにしない）／初期は GSC + LLM まで**。**G1・G2・G3・G6 完了（r61）= 79 ルール**（データ品質 12 / 時系列 12 / クエリ 20 / ページ 14 / デバイス・国 9 / URL 8 / 見え方 4）。残り: G4（GA4 のルール 40 件 + イベント名の対応表）→ G5（GSC × GA4 の 20 件）→ G7（人間による承認）→ G8（CRM） | Claude | **G1〜G3・G6 完了（r61）。次は G4** |
...
- プレミアム（伴走）の中身の詰め: 報告ミーティングの所要時間（いまは 60 分と書いている）、レポート代行の範囲、優先サポートの返信目標（いまは平日 24 時間以内）。紹介サイト・料金画面・特商法にこの内容で出ているので、変えるなら `src/lib/plans/catalog.ts` と `marketing/public/index.html` の両方
...
- 自動診断（#82）で残っている判断: ⑤ 人間による確認の工程（AI の出力を編集して確定させ、承認者を記録する = G7）を入れるか ⑥ CRM 連携（G8）に進むか、進むなら取り込み口（CSV / 手入力 / SFA 連携） ⑦ 閾値の管理画面を作るか（いまは定数 + 目的別プリセット） ⑧ 回数制限をパワーアップ分析と別枠にするか（いまは同じ枠）
