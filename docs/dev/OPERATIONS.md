...
| AI 検索モニタリング（r132 / r142 / r143 / r149 / r151、09-20〜22） | **コード完成・検証済み（lint / tsc / test 1,963 件 / build）。本番で実測が入るのは計測が回ってから（#19 と #91 の 7〜9 が先）** | **画面は 2 カラム（r151）**: 上に自動実行スケジュールのバナー（次回 / 最終実行）とフィルタ行（モデル / タグ / 期間）、左 = 推移 2 枚・出現率 2 枚・**最近の生成結果**（実際の LLM 出力）、右 = 競合シェア・**モデル別（切り替え式 1 枚）**・**ドメイン別の引用**・指名検索・業界の地図・今すぐ実行・クレジット・モデル更新。**計測前は 4 週分のイメージを破線（r149）**。**モデルは 6 つ（r142）**、**業界の地図（r143）**。Supabase のテーブル変更は不要 |
