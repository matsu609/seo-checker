...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r49 | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| 中身を試させたい相手 | https://app.seo-checker.tokyo/ | 無料診断・サイト（ログイン不要。本サービスとは切り離した公開ページ） |
| 店舗の相手に渡す | https://app.seo-checker.tokyo/meo | 無料診断・店舗（ログイン不要） |

無料診断の 2 本（r49）は、専用のヘッダー（ロゴ・ログイン・「初月無料ではじめる」・規約）だけの画面で開く。サイドバーも有料ツールの一覧も出ないので、**そのまま見込み客に渡して構わない**。診断の結果の下に「無料診断で分かるのは『今の状態』まで」→「詳細診断で分かること」→「初月無料で詳細診断をはじめる」（`/sign-up`）が必ず出る。管理画面のサイドバーでは最下部の「お客様に渡す無料診断」から開ける。
