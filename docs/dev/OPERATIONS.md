...
- **Wolf の確認（#5 / #54 ①。09-21 に利用者が着手）の結果。**手順は [google-oauth-verification.md](./google-oauth-verification.md) §7-1（0-1 で「確認済み」のままなら確認作業そのものが不要）。①確認が完了したか（動画確認なら審査 3〜5 営業日） ②完了後に送る再申請の**新しいケース ID** ③却下された場合はその理由（文面ごと共有してもらう）。**確認完了日が元の日付のままか、再確認の日にリセットされるかは Google 側にしか分からない**ので、推測せず申請して結果を見る
