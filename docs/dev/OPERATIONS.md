...
| 135 | **Google サーチコンソール連携（r161）を本番で動かす**（09-24: 手順 1 API 有効・手順 2 スコープ追加 **済み**。残り 3〜5）: 下の「Google サーチコンソール連携を本番で動かす手順」の 1〜5。Search Console API の有効化とスコープ `webmasters.readonly` は 09-11 時点で済んでいたが、r89 以降に外していないかの確認が要る。**OAuth の本番公開審査（#13）の対象に `webmasters.readonly` が戻る**（申請文・デモ動画にサーチコンソール連携を足す） | 利用者 | 未 |
