...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r106 | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| `STRIPE_TRIAL_DAYS` | **未設定のままにする**（r105 から既定 0 = 全員向けトライアルなし。初月無料はクーポンで相手ごとに。もし設定してあれば削除） | 正の数を入れると全員に無料期間が付き、特商法・料金画面の文面もそれに従う。緊急時の逃げ道 |
...
| 1c | **割引コードを 2 種類作る（r105 から初月無料も自動ではなくコード。相手によって使い分ける。テストでは作らず本番で作る）** | https://dashboard.stripe.com/coupons | 下の「割引コード（クーポン）の運用」のとおり。①月額の値引き（例: 金額割引 ¥20,000・期間「永続」）②初月無料（**割引率 100%・期間「1 回」**）。それぞれ「プロモーションコードを追加」で顧客に渡すコードを作る。コードを持たない人は定価のまま（定価表示が二重価格表示にならない） |
...
### 割引コード（クーポン）の運用（r105 から。相手によって使い分ける）

全員向けの無料期間は無い。値引きしたい相手にだけ、Stripe で作ったプロモーションコードを渡す。相手は申し込み画面（Stripe Checkout）の「プロモーションコードを追加」に入力する。コード無しの人は定価。**紹介サイト・アプリの画面・特商法ページには「初月無料」「初回無料」と書かない（r106、利用者の指示 2026-09-18）**。公開している文面は「割引コードをお持ちの場合は申し込み画面で入力」だけ。何のコードを誰に渡すかは営業の場で口頭・メールで伝える。

| 渡したいもの | クーポンの作り方（Stripe（本番）→ 商品カタログ → クーポン →「新規」 https://dashboard.stripe.com/coupons ） | 相手の請求 |
|---|---|---|
| **初月無料** | 名前「初月無料」→ 種類 **割引率 100%** → 期間 **「1 回」（once）** → 保存 →「プロモーションコードを追加」→ コード（例 `FIRSTFREE`）と利用回数の上限・有効期限 | 初回の請求 0 円（カードは登録される）→ 翌月の同じ日から定価 |
| **月額の値引き（ずっと）** | 名前「導入割引 2 万円引き」→ 種類 **金額割引 ¥20,000（JPY）**（または割引率）→ 期間 **「永続」（forever）** | 契約中ずっと割引後の金額 |
| **月額の値引き（最初の n か月だけ）** | 同上で期間 **「複数月」（repeating）**→ 月数 n | n か月だけ割引、以後は定価 |
| 初月無料 + 値引き（併用） | **併用できる**が、Checkout で入れられるコードは 1 つ。A: 相手が初月無料コードで契約 → 運用者が Stripe → 顧客 → サブスクリプション →「割引を追加」で値引きクーポンを付ける（手作業・付け忘れ注意）。C（提案中、未実装）: アプリの `/plans` にコード入力欄を置き、コードごとに「トライアル日数 + Stripe のクーポン ID」を決めて Checkout を作る（Stripe のコード欄は出さなくなる） | A: 1 か月目 0 円 → 2 か月目以降 割引後の額 |

- 作る場所: **本番モード** https://dashboard.stripe.com/coupons （利用者の決定 2026-09-18「テストはいらない。本番一発で運用する」。テスト環境のクーポンは作らない。画面左上が「テストモード」になっていないことを確認してから作る）
- クーポンは**商品を限定しない**で作る（ライトにもスタンダードにも効く）。特定のプランだけに効かせたいなら「適用する商品」で絞る。
- 契約中の人に後から付ける: Stripe → 顧客 → サブスクリプション →「割引を追加」。
- 割引率 100% のクーポンでも Checkout はカードを要求する（2 か月目以降の請求があるため）。「初回 0 円でカード登録だけ」= 旧トライアルと同じ体験になる。
- 特商法ページ（`/legal/tokushoho`）の「お支払い時期」に、初月無料コード使用時の請求の立ち方（初回 0 円、翌月から月額、初回更新日までの解約は無料）を明記済み（r105）。
...
| 5 | 割引コードの発行（本番モードで直接。月額の値引きと初月無料の 2 種類。r105 から初月無料も自動ではない。上の「割引コード（クーポン）の運用」） | 利用者 |
...
| 日付 | 判断 | 理由 |
|---|---|---|
| 割引の併用（初月無料 + 翌月から 1 万円引き）をどの方式でやるか（2026-09-18） | A: Stripe の手作業 2 段階（いますぐ・コード変更なし）／C: アプリに「お客様コード」入力欄を作り、コードごとにトライアル日数 + クーポンを決める（実装 半日。Stripe のコード欄は出さなくなる）。おすすめは C | 利用者の回答待ち |
