...
| AI 検索モニタリングのグラフとモデル（r132 / r142、09-20〜21） | **コード完成・検証済み（lint / tsc / test 1,820 件 / build）。本番で数字が入るのは計測が回ってから（#19 と #91 の 7〜9 が先）** | `/tools/geo` のダッシュボードにグラフを 4 枚。**折れ線 2 枚（r142）**: キーワードごと・プロンプトごとの**週ごとの推移**（直近 8 週、線は最大 6 本を選択式。観測の無い週は 0% ではなく線を切る）。**棒 2 枚（r132）**: いまの水準（4 週ローリング）+ Wilson 95% 信頼区間の帯。**計測モデルは 6 つ（r142）**: ChatGPT / Gemini / **Claude** / **Perplexity** / AI Overviews / **AI モード**。Supabase のテーブル変更は不要。利用者の指示 09-20「キーワードごとに棒グラフ。確率に幅を持たせて分布を」・09-21「Perplexity と Claude と AI モードを追加」「キーワードごとに順位を追うような折れ線に」 |
