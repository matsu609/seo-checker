...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r144（マスター画面の外部連携 18 件・月額費用の試算・設計書。09-21） | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| AI 検索モニタリングのグラフ・モデル・業界の地図（r132 / r142 / r143、09-20〜21） | **コード完成・検証済み（lint / tsc / test 1,840 件 / build）。本番で数字が入るのは計測が回ってから（#19 と #91 の 7〜9 が先）** | `/tools/geo` のダッシュボードにグラフ 4 枚 + カード 1 枚。**折れ線 2 枚（r142）**: キーワード・プロンプトごとの週ごとの推移（直近 8 週、最大 6 本、未計測の週は線を切る）。**棒 2 枚（r132）**: いまの水準（4 週ローリング）+ Wilson 95% 信頼区間の帯。**業界の地図（r143）**: トピックの AI 回答で引用が多いサイトの順位表（LLM Mentions API。押したときだけ・1 回 5 クレジット）。**計測モデルは 6 つ（r142）**: ChatGPT / Gemini / Claude / Perplexity / AI Overviews / AI モード。Supabase のテーブル変更は不要 |
