...
| 66 | **SEO 分析ツール → 「パワーアップ分析」（有料・回数制限つき）**: 利用者の要件書（09-13）を [seo-analysis-spec.md](./seo-analysis-spec.md) に整理。同日の利用者の指示で方針を見直し（§0）: **Google 連携を前提にしない**。URL だけから無料・安価な API で指標（48 ルール + サイトの構成・信頼・CrUX・PSI・SerpApi）を集めて事実シートにし、AI（Claude 主、ChatGPT はセカンドオピニオン）が現状分析と改善案を書く報告書。GSC / GA4 は「連携できたら濃くなる任意の層」。§0.3 の 6 点（回数・ChatGPT の使い方・入力項目・ページ数・順番・画面）の回答待ち → A′ クローラー拡張（構成・信頼指標）から着手 | 利用者 → Claude | **設計まで完了。§0.3 の判断待ち** |
...
- **パワーアップ分析（#66）の 6 点**（[seo-analysis-spec.md](./seo-analysis-spec.md) §0.3。§17 は任意の層に着手するときに改めて）: ①月の回数（推奨: オールインワン 10 回、運営者は無制限）②ChatGPT の使い方（推奨: セカンドオピニオンで食い違う点だけ表示）③入力項目（推奨: URL だけで動き、キーワード・業種・目的・競合は任意）④クロール 300 ページ + PSI 6 本でよいか ⑤着手の順番（推奨: A′ 構成・信頼指標 → B′ 事実シート + AI 分析 + 報告書 + 回数制限 → C′ CrUX → D′ SerpApi → E′ GSC / GA4 の任意層）⑥画面（推奨: SEO タブに `/tools/seo-analysis` を 1 つ追加）
