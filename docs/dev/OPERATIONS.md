...
| 日付 | 判断 | 理由 |
|---|---|---|
| Stripe を本番に切り替えるか（A）、先にテストモードで確認するか（B）（2026-09-18） | 原因判明: Vercel の `STRIPE_PRICE_STANDARD` は本番の Price（`price_1UGM2N…`）、`STRIPE_SECRET_KEY` はテスト鍵で食い違い。A = `sk_live_` + 本番 Webhook の `whsec_` + 本番ライトの Price に揃えて Redeploy（#58 の 8c・8e・8f）。B = `STRIPE_PRICE_STANDARD` をテスト用 `price_1UF73IBQZc3g0qHVJGb0aumu` に戻して確認してから A | 利用者の回答待ち |
