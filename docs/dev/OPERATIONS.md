...
**Stripe の画面でクーポンを作る必要は無い。**必要なクーポン（金額割引・永続・スタンダードの商品限定）は、申し込みのときにアプリが決まった ID（`seo-checker-off<金額>-<商品 ID>`）で自動で作る（`src/lib/billing/coupons.ts`）。無料期間は Stripe のトライアル（30 日）で付ける。テスト鍵と本番鍵で別々に作られる。
