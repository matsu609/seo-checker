...
| 初月無料 + 値引き | Stripe は Checkout で入れられるコードが 1 つなので、**「1 か月目 100%、2 か月目以降 2 万円引き」は 1 つのクーポンでは作れない**。契約後に 顧客 → サブスクリプション →「割引を追加」で 2 つ目を付けるか、初月無料の代わりに「初回 5 万円引き（金額割引・1 回）」を渡す | — |
