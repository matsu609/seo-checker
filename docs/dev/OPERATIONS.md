...
## 現在の状態（2026-09-11 時点。Google 連携の行は 2026-09-23 に更新）
...
| Google Cloud `seo-checker-508104` | OAuth 構成済み（テスト状態） | 下記「Google Cloud の設定」。要求するスコープは口コミ返信の `business.manage` と、**r161 で再開した `webmasters.readonly`（サーチコンソール連携）**。GA4 は廃止のまま |
| Google 連携（GSC） | **再開（r161、09-23。利用者の指示「SEO のタブに Google サーチコンソール連携というタブを作ってもう一度実装」）** | SEO の柱に `/tools/search-console`（ライト）。接続 → サイト選択 → 実測を 1 画面で。**本番で動かすには Google Cloud 側の確認が要る（#135）**。GA4 は提供終了のまま |
| Google 連携（GA4） | **提供終了（r89、09-17。利用者の決定）** | 代替: 検索パフォーマンス（推定）（r87）。サイト内の行動（訪問者・CV）は外部から取れず、自前タグも r90 で取り下げ。コードは r93 で削除済み |
...
| 定期処理（`/api/cron/daily`、r127） | **停止中（r163、09-24。利用者の指示「定期処理をすべて止めて」）。`vercel.json` の `crons` を空にした = `/api/cron/daily` と `/api/cron/geo-run` の 2 本とも自動では動かない。再開は `crons` を戻すだけ（下の作業ログ 09-24 に元の値）。マスター画面の「今すぐ実行」は手動で使える。**（09-20 に本番で動作確認済み） | 毎日 5:00 JST。月: マップ診断 / 火: 順位計測 / 水: サイト監視 / 1 日: 月次レポート / 2 日: 掲載の再チェック / 毎日: 投稿の送信・自動再診断。記録はマスター画面の「定期処理（Cron）の状況」 |
...
| 135 | ~~**Google サーチコンソール連携（r161）を本番で動かす**~~ **完了（09-24。利用者「うまく機能してました」）**。以下は記録（手順 1〜5）: 下の「Google サーチコンソール連携を本番で動かす手順」の 1〜5。Search Console API の有効化とスコープ `webmasters.readonly` は 09-11 時点で済んでいたが、r89 以降に外していないかの確認が要る。**OAuth の本番公開審査（#13）の対象に `webmasters.readonly` が戻る**（申請文・デモ動画にサーチコンソール連携を足す） | 利用者 | 未 |
...
### Google サーチコンソール連携を本番で動かす手順（#135。r161 の反映後）
...
- Clerk の設定は変えなくてよい（スコープは画面のボタンがその場で要求する。Google 連携が「独自のクレデンシャル」なのは 09-11 から）。
- 費用: Search Console API は無料。1 回の取得で 5 本呼ぶ（合計・前期間・日別・キーワード別・ページ別）。同じ条件は 10 分キャッシュ（利用者ごと）。
...
| 09-24 | **サーチコンソールの AI 分析は月 1 回・持ち越しなし。回数の判定は Clerk の privateMetadata（前回の作成日時）で行い、usage_events は表示用の記録だけ**（r162） | 利用者の指示「分析ボタンを設けて月に 1 回クレジットを付与して、1 枚以上たまらない」。既存の月の上限（takeUsage）は **Supabase が無い・テーブルが無いと通してしまう（fail open）**ので、1 回きりの枠には弱い。前回の結果を保存する場所（Clerk）に日時も入っているので、そこで判定すれば DB に依存しない。**AI が失敗したときは消費しない**（1 回しか無いので）。押し間違い防止に 1 段の確認を挟んだ。出力は「現状分析 + ネクストアクション 3 件（対象のページ / キーワード + やること）」で合計 400 文字程度（厳密な文字数の強制はしない。画面に文字数を出す） |
| 09-23 | **Google サーチコンソール連携を、SEO の柱のタブとして作り直す**（r161）。**GA4 は戻さない。精密診断にも取り込まない（まず画面だけ）** | 利用者の指示「以前つけた Google サーチコンソール連携を、SEO のタブに『Google サーチコンソール連携』というタブを作ってもう一度実装」。09-17 の「お客様側の作業が要る機能は置かない」の**例外**として、**任意の連携**に置く（連携しないお客様は推定で数字が出るので、契約初日から空にならない）。旧実装は設定画面でサイトを選ばせていたが、**接続・選択・表示を 1 画面で完結**させた（設定画面と行き来させない）。保存時に sites.list と照合して、他人のサイト URL を保存させない（旧実装は形式しか見ていなかった）。キャッシュも利用者ごとにした |
...
### 2026-09-23（利用者の指示「SEO のタブに Google サーチコンソール連携を作ってもう一度実装」→ r161）

- **経緯の確認**: GSC 連携は r89（09-17）で利用者の決定により提供終了、r93 でコードごと削除していた。削除前のコミット（`28eef77^`）から、解析・クライアント・画面を持ち出して今の作りに組み直した。
- **作ったもの**: `/tools/search-console`（SEO の柱・成果のくくり・ライト。サイドバーは「順位計測」の次）。①接続（`webmasters.readonly`。既に許可された口コミ返信の権限も一緒に要求）②対象サイトの選択（所有権確認済みのみ。保存時に sites.list と照合）③期間 7〜365 日の合計と前期比・日別クリック数の折れ線・キーワード / ページの上位 100 件。連携前はイメージ（破線）を出す。使えるサイトが無いときは登録手順を案内。
- **API**: `POST /api/search-console/site`、`POST /api/search-console/performance`。コードは `src/lib/google/search-console/`・`src/components/search-console/`。
- **文言を合わせた所**: プライバシーポリシー第 5 条（最終更新日を 09-23 に）、料金表のライトの説明、検索の推定画面の注記、精密診断の Google 層の案内、README・ARCHITECTURE・tool-map・設計書（blueprint）。**プライバシーポリシーは #12 の専門家レビューでも見てもらう**。
...
- `webmasters.readonly` は 09-11 の記録では「非機密のスコープ」の欄に出ていた。追加後にどちらの欄に出るかを見て、本番公開審査（#13）の申請文に加える。テスト状態のうちは審査なしで使える（テストユーザーのみ・トークン 7 日）。

- 09-24 続き: 利用者が URL を Google Cloud 画面上部の検索窓に貼ってしまい、検索結果が出た。「スコープを追加または削除」で開く右のパネルを下までスクロールした先の「スコープの手動追加」の入力欄に貼るよう、改めて案内した。
- 09-24 続き: 利用者「できた」。データアクセスに `webmasters.readonly` を追加・保存済み（手順 2 完了）。残りは手順 3（テストユーザー）・4（Search Console の権限）・5（アプリで接続して数字が出るか・口コミ返信が動くか）。

### 2026-09-24（利用者「うまく機能していた。ただ表示しただけで分析できていないので、月 1 回だけ回せる分析ボタンを。400 文字で現状分析とネクストアクション」→ r162）

- #135 は完了（本番でサーチコンソール連携が動いた）。
- **作ったもの**: サーチコンソール連携の画面に「AI の現状分析とネクストアクション」カード。「AI で分析する（今月あと 1 回）」→ 確認 →「分析する」。いま選んでいる期間（既定 28 日）の実測を Claude に読ませ、現状分析 + ネクストアクション 3 件を合計 400 文字程度で出す。結果は保存され、開き直しても出る。使ったあとは「次は ○月 1 日から」。
- **回数**: 日本時間の月ごとに 1 回、持ち越しなし。運用者（ADMIN_EMAILS）は数えない。AI の失敗・表示回数 0 のときは消費しない。
- **API**: `POST /api/search-console/analysis`。コードは `src/lib/google/search-console/{analysis,analyze,performance}.ts`。設定画面の「今月の利用回数」に「サーチコンソールの AI 分析」が出る（Supabase に記録できたとき）。
- **前提**: 本番の `ANTHROPIC_API_KEY`。未設定なら画面に「設定が必要」と出てボタンが出ない。
- **検証**: lint / tsc / test 1,956 件 / build 通過。本番での AI 出力の中身は未確認（利用者に 1 回押してもらう。運用者アカウントなら回数を消費しない）。

### 2026-09-24（大規模リファクタリングとバグ調査・定期処理の停止、r163）

- **利用者の依頼（09-23）**: 「大規模なリファクタリングをしてほしい。バグや矛盾があれば報告してほしい」。
- **調査**: 6 分野（API ルート / 料金・権限 / 診断ロジック / MEO・Google / AI 検索モニタリング・定期処理 / 重複コード）を並行で洗い出した。確認できた不具合は 100 件超。
- **修正の状況（作業ブランチ `claude/tender-goldberg-gvwuzz` = `57b9f82`。main には未マージ・本番未反映）**:
  - 入っているもの: MEO（複数店舗で最新の報告書が欠ける・一括登録で営業時間を壊して Google に上書き・掲載の再チェックが月によって飛ぶ・投稿の二重送信・NAP の誤判定・代理ログイン中に Google へ書き込める 等）、AI 検索モニタリングと定期処理（キャッシュの衝突・反復の水増し・月初のクレジットリセットが消える・SEO 順位が常に圏外・契約切れでも計測・月次ジョブが丸 1 か月飛ぶ・月次メールの二重送信 等）、画面の共通部品の整理。lint / tsc / test 2,143 件は通過、**build は未確認**。
  - **途中で止めたもの（利用者の指示で中断）**: 診断ロジック・SSRF 対策（班 C）、料金・権限・API（班 D）。作業は取り込んでいない（ローカルのみ・消えてよい）。
- **定期処理の停止（r163）**: 利用者の指示「定期処理をすべて止めて」。`vercel.json` の `crons` を空にした。元の値は `[{"path":"/api/cron/daily","schedule":"0 20 * * *"},{"path":"/api/cron/geo-run","schedule":"0 20 * * *"}]`。止まるもの: 予約投稿の送信・マップの週次更新・順位計測・サイト監視・月次レポート・掲載の再チェック・精密診断の自動再診断・AI 検索モニタリングの計測。**予約済みの投稿は自動では送られない**。
- **入力待ち（利用者の判断）**:
  1. AI 検索モニタリングの定期計測（ChatGPT / Gemini / Claude）は DataForSEO の標準キュー（task_post）に投げたまま結果を取りに行っておらず、**毎日失敗して実費だけ出ていた**（Perplexity は Live なので正常）。直し方は「Live に切り替える（単価 $0.0012 → $0.004）」か「翌日の実行で回収する（tasks_ready + task_get）」の二択。
  2. 精密診断がライトに含まれるか: 料金表・紹介サイトは「含む」（r71）、実際の制限はスタンダードから。どちらに合わせるか。
  3. 定期処理をいつ再開するか。
