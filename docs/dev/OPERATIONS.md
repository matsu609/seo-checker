...
| 78 | **パワーアップ分析を動かすための設定**（下の表「パワーアップ分析を有効にする手順」）: ~~Supabase の SQL~~ → Chrome UX Report API の有効化とキーの制限追加 → 本番で試す → `SEO_ANALYSIS_MONTHLY_LIMIT` は既定 10 のままでよいか | 利用者 | **1・2 完了（09-15。`analysis_runs` を作成、Chrome UX Report API を有効化）**。**1b 完了（09-15、`audit` 列を追加）**。残り 3（キーの制限に追加）・4（本番で 1 回実行） |
