...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r133（管理アカウントの全機能開放・顧客管理の切り出し。09-20 にマージ・push 済み） | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| 権限の線引き（r133、09-20） | **main にマージ済み（Vercel が自動デプロイ）。検証は lint / tsc / test 1,808 件 / build 通過。本番での通し確認は未（#94）** | 利用者の指示 09-20。**管理アカウントはカードの登録なしでツールを全部使える**（運用者と同じ扱い。判定は `checkPlanForFeature` / `canUseFeature` / `accessAllows` の 3 か所）。マスターとの違いは 2 つだけ: ①マスター画面（`/admin` = 版・外部連携・定期処理・管理アカウントの追加）が見えない ②見えるお客様は担当分だけ。お客様の契約状況・ご利用状況・ご意見への返答は**顧客管理（`/clients`）**に切り出し、サイドバーのタブ名は「運用」→「**管理者用**」。無料診断のデモ枠（月 50 回）は据え置き |
...
| 94 | **管理アカウントの本番確認**（r77 → r133 で中身が変わった）: マスター画面で管理アカウントを 1 件追加 → **顧客管理（`/clients`）**で担当を割り当て → その管理アカウントでログインして「顧客管理」に担当分だけが出て、**ツールが鍵なしで全部開くこと**・**マスター画面のリンクが出ないこと**を確認。手順は下の「管理アカウントを使いはじめる手順」 | 利用者 | 未（コードは完了） |
...
### 管理アカウントを使いはじめる手順（#94。利用者の作業。r133 の反映後）

管理アカウントに、担当としてお預かりしているお客様だけをお見せするための設定です。**環境変数は要りません**（マスターの判定に使う `ADMIN_EMAILS` は設定済み）。r133 が本番に出ている（#126 のマージ済み）ことを確認してから行ってください。

| # | サービス・画面 | URL | やること |
|---|---|---|---|
| 1 | 本番 → マスター画面 | https://app.seo-checker.tokyo/admin | ページ上部の「動いているコミット」が main の先頭（r133 以降）になっていることを確認 |
| 2 | 本番 → マスター画面 → 「管理アカウント」 | https://app.seo-checker.tokyo/admin | 管理アカウントにする方のメールアドレスを入れて追加。**すでに登録済みの方はその場で管理アカウントになり、未登録の方には Clerk から招待メールが飛びます**（相手が登録を済ませると一覧に並びます） |
| 3 | 本番 → 顧客管理 → 顧客一覧 | https://app.seo-checker.tokyo/clients | お客様の行の「担当の管理アカウント」で、手順 2 の方を選ぶ。押した瞬間に保存されます（「担当なし」に戻せば見えなくなります） |
...
| 5 | 本番 → 顧客管理（**管理アカウントでログインして**） | https://app.seo-checker.tokyo/clients | 手順 3 で割り当てたお客様だけが並ぶことを確認。サイドバーの「**管理者用**」に「顧客管理」が出ます（「マスター画面」は出ません） |
| 6 | 本番 → ツール（**同じく管理アカウントで**） | https://app.seo-checker.tokyo/tools/seo-analysis | **カードの登録なしでツールが全部開くこと**（サイドバーに鍵バッジが出ないこと）を確認 |

**注意していただきたいこと**

- 管理アカウントは**ツールを全部使えます**（カードの登録は不要。r133、利用者の指示 2026-09-20）。マスターとの違いは「マスター画面が見えない」ことと「見えるお客様が担当分だけ」の 2 つです。
- 管理アカウントにできるのは、担当のお客様への **ご意見への返答・割引・機能の個別開放・その方の画面を見る（代理ログイン）**です。**プランの変更と担当の割り当てはできません**（運用者だけ）。担当外のお客様を指定しても 404 になります。
- 管理アカウントには**契約状況・月額・プラン・次回請求・登録日・最終利用日・登録情報・ご意見**が見えます。月額を見せたくない場合はお知らせください（外せます）。
- 「解除」を押すと、その場でその方は顧客管理を開けなくなり、**ツールも契約どおりの範囲に戻ります**。担当の割り当て自体は残るので、管理アカウントに戻せば担当もそのまま戻ります。
...
- **Wolf の確認（#5 / #54 ①。09-21 に利用者が着手）の結果。**手順は [google-oauth-verification.md](./google-oauth-verification.md) §6-7。①確認が完了したか（動画確認なら審査 3〜5 営業日） ②完了後に送る再申請の**新しいケース ID** ③却下された場合はその理由（文面ごと共有してもらう）。**確認完了日が元の日付のままか、再確認の日にリセットされるかは Google 側にしか分からない**ので、推測せず申請して結果を見る
