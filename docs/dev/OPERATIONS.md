...
| サービス | 状態 | 備考 |
|---|---|---|
| GitHub `matsu609/seo-checker` | main = r107 | main に push すると Vercel が自動デプロイ。紹介サイトのソース `marketing/` も同居（09-10 に統合） |
...
| `PROMO_CODES` | **未設定（r107 で新設。利用者の作業）** | 割引コードの一覧 `CODE=pattern`（カンマまたは改行区切り）。パターンは下の「割引コードの運用」。設定して Redeploy すると `/plans` の料金表の上に入力欄が出る |
...
| 1c | ~~Stripe でクーポンを作る~~ → **r107 から不要**。割引コードは Vercel の環境変数 `PROMO_CODES` に書く（下の「割引コードの運用」）。クーポンはアプリが自動で作る | https://vercel.com/matsumatsu452-6233/seo-checker/settings/environment-variables | `PROMO_CODES` に `コード=パターン` を並べて Redeploy |
...
### 割引コードの運用（r107 から。スタンダード専用・10 パターン。相手によって使い分ける）

全員向けの無料期間は無い。値引きしたい相手にだけ「割引コード」を渡す。相手はログイン後の料金プラン（`/plans`）の料金表の上にある「割引コードをお持ちの方」にコードを入れて「確認」→ 内容（例「最初の 30 日間は無料 + 月額 20,000 円引き」）が出る → スタンダードの「申し込む」→ Stripe Checkout に割引と無料期間が付いた状態で進む。コード無しの人は定価。**紹介サイト・アプリ・特商法には「初月無料」「初回無料」と書かない**（r106、利用者の指示）。公開文面は「割引コードをお持ちの場合は料金プランの画面で入力（スタンダードのみ）」だけ。

**Stripe の画面でクーポンを作る必要は無い。**コードごとに必要なクーポン（金額割引・永続・スタンダードの商品限定）は、申し込みのときにアプリが決まった ID（`seo-checker-off<金額>-<商品 ID>`）で自動で作る（`src/lib/billing/coupons.ts`）。無料期間は Stripe のトライアル（30 日）で付ける。テスト鍵と本番鍵で別々に作られる。

| パターン名（`PROMO_CODES` に書く） | 相手の請求（定価 50,000 円） |
|---|---|
| `off10` / `off20` / `off30` / `off40` | 毎月 40,000 / 30,000 / 20,000 / 10,000 円（ずっと） |
| `off50` | 毎月 0 円（ずっと無料。カードは登録される） |
| `free` | 最初の 30 日間 0 円 → 30 日後から毎月 50,000 円 |
| `free-off10` / `free-off20` / `free-off30` / `free-off40` | 最初の 30 日間 0 円 → 30 日後から毎月 40,000 / 30,000 / 20,000 / 10,000 円（ずっと） |

- 設定の仕方: Vercel の環境変数 `PROMO_CODES` に `コード=パターン` をカンマか改行で並べる。例: `WOLF-A7K2=off10, TANAKA-Q9=free-off20, ZERO-K3M8=off50`。保存したら Redeploy。コードは大文字小文字と空白を区別しない。**同じパターンに何個でもコードを付けられる**ので、相手ごとに別のコードを渡し、渡した相手をこのメモか台帳に控える。
- コードは**推測されにくい長さ（8 文字以上、英数字 + ハイフン）**にする。とくに `off50`（永続無料）と `free-off40` は当てられると損失が大きい。`/api/billing/promo` はログイン必須だが回数制限は無い。
- コードを止める: `PROMO_CODES` からその行を消して Redeploy（既に契約した人の割引は Stripe のサブスクリプションに付いているので影響しない）。
- 契約中の人の割引を変える・外す: Stripe（本番）→ 顧客 → サブスクリプション → 割引の欄 https://dashboard.stripe.com/customers
- コードはスタンダード専用。ライトの「申し込む」にはコードが付かず、サーバーもコード付きのライトを拒否する（`/api/billing/checkout` が 400）。
- 全員向けに無料期間を戻したいときだけ `STRIPE_TRIAL_DAYS`（正の数）。通常は未設定。
...
| 5 | 割引コードの発行 = Vercel の `PROMO_CODES` に書いて Redeploy（r107。Stripe でクーポンを作る作業は不要。上の「割引コードの運用」） | 利用者 |
