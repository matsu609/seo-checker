...
| 116 | **Performance API を承認当日に動かすための利用者の作業**: 下の「Business Profile Performance API を使えるようにする手順（#116）」の表 | 利用者 | **09-18: Business Profile Performance API・My Business Account Management API・My Business Business Information API の 3 本は有効化済み（利用者報告）。Google My Business API（v4）は直リンクが開かない = 承認待ち（承認後に開き直す）。**残り: スコープ確認（3）・テストユーザー（4）・Clerk の名前（7）・ブランディング（8）・ケースの督促（6） |
