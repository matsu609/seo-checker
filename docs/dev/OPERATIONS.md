...
| 82 | **GSC / GA4 / CRM の自動診断 + コンサル回答生成**: 仕様は [diagnosis-rules-spec.md](./diagnosis-rules-spec.md)。利用者の決定（09-15）= **入口は既存の Google 連携（OAuth）のみ・CSV は作らない／パワーアップ分析と同義（別ツールにしない）**。**G1・G2・G3・G6 完了（r61）= GSC 79 ルール**。**G4 完了（r65）= GA4 47 ルール + イベント名の共通化。合計 126 ルール**。残り: G5（GSC × GA4 の 20 件、1〜2 日）→ G7（人間による承認）→ G8（CRM） | Claude | **G1〜G4・G6 完了（r65）。次は G5** |
