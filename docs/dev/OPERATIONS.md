...
- **次に作る AIO 機能（09-19 の提案の残り）**: 1. 掲載タブの絞り込み（r127 で完了） 2. GBP 投稿の配信（r127 で完了。#5 の承認後に動く） **3. 掲載の生存監視（`evidence_url` / `last_checked_at` / `next_check_at`） 4. 構造化データ（JSON-LD）の生成**。3 か 4 のどちらから着手するか指示待ち
