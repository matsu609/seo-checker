...
- **掲載の手順が多すぎる問題（09-19 の相談）**: ① 顧客の画面を 8 媒体に絞るか（`manual` 13・`monitor` 14 を畳む） ② `manual` の作業を `/agency`・`/admin` の作業画面に移して**代行（有料オプション）**にするか ③ Uberall 等の OEM を買うか。**①はすぐやれる。②③は利用者の判断待ち**
- **次に作る AIO 機能の順番（09-19 の提案）**: 1. 掲載タブの絞り込み 2. GBP 投稿の配信（`localPosts`。#5 の承認後に効く） 3. 掲載の生存監視（`evidence_url`） 4. 構造化データ（JSON-LD）の生成。**どれから着手するか指示待ち**
