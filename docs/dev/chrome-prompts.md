...
**前提が 2 つある。どちらも満たさないうちに送り直しても同じ失敗になる。**

1. **r84（robots.txt の修正）が本番に出ていること。**`https://app.seo-checker.tokyo/admin` の「バージョン」カードで、
   動いているコミットが `1d316f7` 以降（= `210f868` のマージを含む）かどうかを見る。
2. **Google が新しい robots.txt を読み直していること。**Google は robots.txt を**最大 24 時間**キャッシュする。
   修正を出した直後に送り直しても、Google の手元は古い（`Disallow: /` で塞がった）ままのことがある。
   Search Console の **設定 → robots.txt レポート**で、Google が持っている中身と最終読み込み日時を確認し、
   古ければ「再クロールをリクエスト」する。

**先に原因を確定させたいときは、送り直すより URL 検査が速い。**画面上部の検査窓に
`https://app.seo-checker.tokyo/sitemap.xml` を入れると、robots.txt で拒否されているかどうかがはっきり出る。
...
### 1. 修正が本番に出ているか確かめる
https://app.seo-checker.tokyo/admin を開く。「バージョン」カードに出ている「動いているコミット」を読む。
それが 1d316f7 と同じか、それより新しければ次へ進む。古ければ**ここで止めて報告する**（まだ直っていないので、送り直しても無駄になる）。

### 2. Google がいま持っている robots.txt を確かめる
1. https://search.google.com/search-console/settings?resource_id=sc-domain%3Aseo-checker.tokyo を開き、「robots.txt」のレポートを開く。
2. app.seo-checker.tokyo の robots.txt の中身と「最終読み込み日時」を読む。
3. 中身に「Allow: /sitemap.xml」の行が**無ければ**、Google は古い robots.txt を持っている。その画面から「再クロールをリクエスト」できるなら押す。
4. 中身と最終読み込み日時を報告する。

### 3. サイトマップの URL を検査する
画面上部の検査窓（「seo-checker.tokyo」内のすべての URL を検査）に次を貼って Enter。
https://app.seo-checker.tokyo/sitemap.xml
出た結果をそのまま報告する。「robots.txt により拒否されました」と出るかどうかが要点。
...
## 報告してほしいこと
- 手順 1 の「動いているコミット」。
- 手順 2 の robots.txt の中身（全文）と最終読み込み日時。「再クロールをリクエスト」を押せたかどうか。
- 手順 3 の URL 検査の結果（画面の文言をそのまま）。
- 手順 4 のあと、一覧の 2 行それぞれについて「URL / ステータス / 検出されたページ数」。
...
**すぐに成功にならなくても、robots.txt の中身さえ正しければ放っておいてよい。**Google は失敗したサイトマップを
自分で再試行する。手順 2 の robots.txt に `Allow: /sitemap.xml` が出ていて、手順 3 の URL 検査が
「robots.txt により拒否されました」と言わなくなっていれば、あとは時間の問題。
