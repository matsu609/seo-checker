...
アプリ本体にある `public/service-guide.html` は別物です（「サービス資料」の静的版。
`NEXT_PUBLIC_SERVICE_GUIDE_URL` を設定したときだけ配られ、未設定ならアプリが
`src/components/free/ServiceGuide.tsx` で組み立てた PDF を配ります）。片方を直しても他方は変わりません。
**静的版は機能レジストリを読まないので、機能を増減したら手で直す**（2026-09-22 の FAQ 提案・
AI ライティングの引退で、2 世代ぶんまとめて直した）。
...
CTA はすべて `https://app.seo-checker.tokyo/`（クイック診断）、`/sign-in`、`/plans` を指す。

## 無料 AIO 診断での自己採点

`https://app.seo-checker.tokyo/` のクイック診断でこのページを採点し、指摘を潰す運用にしている。
