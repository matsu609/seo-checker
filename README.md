...
運用者と管理アカウントが開ける画面です。お客様からお問い合わせ・クレームが来たとき、この 1 画面で終わるようにしてあります。**どちらの立場でも、登録しているすべてのお客様**が新しい順に並び（2026-09-21 の決定。担当による絞り込みはしません）、1 人ごとに次を確認・操作できます。
...
| 管理アカウント | Clerk の `publicMetadata.role` が `agency` | 登録しているすべてのお客様 | ご意見への返答、割引、機能の個別開放、代理ログイン。**マスター画面は見えない**（他の管理アカウントには触れない） |
| 登録者（お客様） | 上のどちらでもない | 自分のツール画面だけ | — |

**ツールはカードの登録なしで全部使えます**（2026-09-20 の決定）。契約（プラン）の判定を通さず、運用者と同じ扱いで開きます。運用者との違いは「**マスター画面（システム・バックエンド）が見えないこと**」だけです。無料診断はデモ用の枠（月 50 回）のままです。
