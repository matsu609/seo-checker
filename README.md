...
顧客一覧の上には**「お客様からのご意見・不具合」**のカードがあり、ツールの右上から届いた報告を新しい順に出します（Supabase の `feedback`）。状態（未対応 / 対応中 / 対応済み）と返答をその場で書け、返答はお客様の設定画面「ご意見の履歴」に出ます。
...
### 管理アカウント画面（`/agency`。旧称: 代理店画面。2026-09-18 に改称）
...
**管理アカウントを追加する**（マスター画面の「管理アカウント」）: メールアドレスを入れて「代理店として追加」を押します。
...
「お客様の画面で何が表示されているか」を、説明を聞かずに自分の目で確かめるための機能です。マスター画面の各お客様の右上にある **「この方の画面を見る」** を押すと、そのお客様としてログインした状態になります。トラブルのお問い合わせや、表示の確認に使います。
...
| 歯止め | 中身 |
|---|---|
| 入れる人 | `ADMIN_EMAILS` の運用者だけ（`requireAdmin()`）。他の人には 404 |
...
**自分に戻るとき**は、その帯の「終了して自分に戻る」を押します。代理中はサイドバーの「マスター画面」が消える（判定がお客様のアカウントで行われるため）ので、**戻る入口はこの帯だけ**です。Clerk が複数セッションを持てる設定なら自分のセッションに戻り、そうでなければログイン画面に出るので、もう一度ご自分でログインしてください。
