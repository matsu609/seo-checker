...
顧客一覧の上には**「お客様からのご意見・不具合」**のカードがあり、ツールの右上から届いた報告を新しい順に出します（Supabase の `feedback`）。状態（未対応 / 対応中 / 対応済み）と返答をその場で書け、返答はお客様の設定画面「ご意見の履歴」に出ます。運用者にも管理アカウントにも全員分が出ます。

顧客 1 人への操作（割引・機能の個別開放・代理ログイン・ご意見への返答）は、サーバー側で必ず `requireClientAccess()` を通ります。**他の管理アカウント**を指定したときだけ **404** です（管理アカウントどうしで権限や金額を付け合えないようにするため）。
...
|---|---|---|---|
| マスター（運用者） | 環境変数 `ADMIN_EMAILS` の**確認済み**メール | 登録しているすべてのお客様 | 管理アカウントの追加・解除、ご意見への返答、割引、機能の個別開放、代理ログイン。システム側（`/admin`）も |
| 管理アカウント | Clerk の `publicMetadata.role` が `agency` | 登録しているすべてのお客様（管理アカウント自身は一覧に出ない） | ご意見への返答、割引、機能の個別開放、代理ログイン。**マスター画面とお客様向けのツールは見えない** |
