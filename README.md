...
運用者と管理アカウントが開ける画面です。お客様からお問い合わせ・クレームが来たとき、この 1 画面で終わるようにしてあります。**運用者には登録しているすべてのお客様**が、**管理アカウントには担当として割り当てられたお客様だけ**が新しい順に並び、1 人ごとに次を確認・操作できます。
...
| **担当の管理アカウント（選択。運用者だけ）** | Clerk の `publicMetadata.agencyId` |
| **割引（スタンダード専用の 10 パターン）** | Clerk の `publicMetadata.promo` |
| **この方の画面を見る（代理ログイン）** | Clerk の Actor Token |

顧客一覧の上には**「お客様からのご意見・不具合」**のカードがあり、ツールの右上から届いた報告を新しい順に出します（Supabase の `feedback`）。状態（未対応 / 対応中 / 対応済み）と返答をその場で書け、返答はお客様の設定画面「ご意見の履歴」に出ます。運用者には全員分、管理アカウントには**担当のお客様の分だけ**が出ます。

顧客 1 人への操作（割引・機能の個別開放・代理ログイン・ご意見への返答）は、サーバー側で必ず `requireClientAccess()` を通ります。管理アカウントが担当外のお客様を指定しても **404** です（そのお客様の存在自体を教えません）。
...
お客様の対応を手伝っていただく方のアカウントです。運用者（マスター）がマスター画面で追加し、どのお客様を担当するかを顧客管理の画面で割り当てます。

| 役割 | 決まり方 | 見えるお客様 | できること |
|---|---|---|---|
| マスター（運用者） | 環境変数 `ADMIN_EMAILS` の**確認済み**メール | 登録しているすべてのお客様 | 管理アカウントの追加・解除、担当の割り当て、ご意見への返答、割引、機能の個別開放、代理ログイン。システム側（`/admin`）も |
| 管理アカウント | Clerk の `publicMetadata.role` が `agency` | 自分に割り当てられたお客様だけ | ご意見への返答、割引、機能の個別開放、代理ログイン。**マスター画面は見えない** |
| 登録者（お客様） | 上のどちらでもない | 自分のツール画面だけ | — |

**ツールはカードの登録なしで全部使えます**（2026-09-20 の決定）。契約（プラン）の判定を通さず、運用者と同じ扱いで開きます。違いは「マスター画面（システム・バックエンド）が見えないこと」と「見えるお客様が担当分だけであること」の 2 つだけです。無料診断はデモ用の枠（月 50 回）のままです。
...
**担当を割り当てる**（顧客管理の顧客一覧）: お客様ごとの「担当の管理アカウント」で選ぶと、その方の画面に出るようになります。「担当なし」に戻せば見えなくなります。

プランの変更と担当の割り当ては運用者だけに残してあります。管理アカウントを「解除」すると、その場で顧客管理の画面が開けなくなります（404）。担当の割り当て自体は消さないので、管理アカウントに戻せば担当もそのまま戻ります。

**マスターだけ環境変数**にしてあります。運用者の権限を Clerk 側の値に置くと、Clerk を触れる人が自分を運用者にできてしまうためです。管理アカウントと担当は Clerk の `publicMetadata` に持ちますが、ここは Backend API からしか書けないので、お客様がご自分で管理アカウントに化けたり担当を付け替えたりはできません。

> Clerk の Backend API には「publicMetadata で絞る」条件が無いため、管理アカウントの一覧と担当分は読んでから絞っています。読む人数は 500 人で打ち切ります（`src/lib/admin/clients.ts` の `MAX_SCAN`）。これを超える規模になったら、担当の関係だけ Supabase に持たせて絞り込みをデータベース側に移してください。
...
| 歯止め | 中身 |
|---|---|
| 入れる人 | 運用者は全員に、管理アカウントは**担当のお客様だけ**（`requireClientAccess()`）。他の人には 404 |
