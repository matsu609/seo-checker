...
| `OPENPAGERANK_API_KEY` | ドメインパワーの「外部からのリンクの評価」（Open PageRank。無料。未設定なら他の指標だけで採点） |
