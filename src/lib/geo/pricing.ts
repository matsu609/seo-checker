 ...
 */
import type { MeasurementKind, RunMode } from "./types";
...
export function costUsd(kind: MeasurementKind, mode: RunMode, prices: UnitPrices = unitPrices()): number {
  if (kind === "rank") return prices.rank;
  if (kind === "aio") return prices.aio;
  return mode === "live" ? prices.llmLive : prices.llmStandard;
}
