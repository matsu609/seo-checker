...
export const PLANS: readonly Plan[] = [
  ...
  },
  {
    ...
    highlights: [
      ...
      "AIO: ページ最適化レポート・AIO 頻出トピック・LLMO モニタリング・プロンプト拡張",
      ...
      "Search Console・Google アナリティクスの初期設定はお客様側で行っていただきます（設定の代行はプレミアム）",
      "初月無料。お申し込み時はカードのご登録だけで、無料期間中に解約すれば料金は発生しません",
    ],
  ...
  },
  {
    ...
    highlights: [
      ...
      "Search Console と Google アナリティクスの初期設定の代行（ドメインの登録・所有確認、GA4 の計測タグの設置のご案内、連携の設定まで）",
      "Search Console の実測値（検索パフォーマンス）。連携の設定を代行したうえでお出しします。ライトでは連携の要らない推定値をお使いいただけます",
      "Google アナリティクス（GA4）を使う分析（サイトレポート・生成 AI 流入分析）。GA4 の計測タグがサイトに入っていることが前提のため、ご相談のうえ有効にします",
