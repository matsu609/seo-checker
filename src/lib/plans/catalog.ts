 ...
 *   light    … 「ライト」月額 38,000 円。診断と計測だけ。AI が改修案を作るツールは付かない（= 意図的に物足りない段）
 *   standard … 「スタンダード」月額 50,000 円。本命。ライトのすべて + AI が「そのまま貼れる改修案・FAQ・返信文」まで作る（= AI がコンサルする段）
 *   premium  … 「プレミアム（伴走）」月額 **150,000 円〜**。スタンダードのすべて + 人の作業（月 1 回の報告ミーティング・
 *              レポート代行・優先サポート）。運営者の時間が要るので月 3 社まで。
 ...
 * 「AI が作るツール」がまるごと落ちる、という線の引き方にしてある。これは
...
export const PLANS: readonly Plan[] = [
  ...
  },
  {
    ...
    label: "ライト",
    priceYen: 38_000,
    summary: "現状を正しく知るための段階。SEO・AIO・MEO の診断と計測がすべて使えます（AI が現状分析と改善案を書く精密診断も含みます）。AI が改修案そのもの（貼れる文章・FAQ）を作るツールは含みません。",
    highlights: [
      "SEO: 精密診断（サイト全体の診断 + AI の現状分析と改善案）・ページ診断・順位計測・検索パフォーマンス（推定。Google 連携なしで、その日から数字が出ます）・キーワード調査・Google サーチコンソール連携",
      ...
      "AI が改修案そのもの（貼れる文章・FAQ・返信文）を作るツールは含みません。精密診断で「どこをどう直すか」の方針までは出ますが、貼る文章の作成はご自身で行っていただく形になります",
