...
export const PLANS: readonly Plan[] = [
  ...
  },
  {
    ...
    highlights: [
      "SEO: 精密診断（サイト全体の診断 + AI の現状分析と改善案）・ページ診断・順位計測・検索パフォーマンス（推定。Google 連携なしで、その日から数字が出ます）・アクセス解析（1 行の計測タグを貼るだけ。訪問者・流入元・電話やメールのタップ）・キーワード調査",
      ...
      "Google Search Console・Google アナリティクスの設定は不要です（本サービスは使いません。検索は推定値、サイト内の行動は計測タグで取ります）",
      "初月無料。お申し込み時はカードのご登録だけで、無料期間中に解約すれば料金は発生しません",
    ],
  ...
  },
  {
    ...
    highlights: [
      ...
      "計測タグの設置の代行（ホームページの管理会社との調整を含みます）",
