...
export const INTEGRATIONS: Record<IntegrationKey, IntegrationMeta> = {
  anthropic: {
    ...
    description: "FAQ 生成・LLM サマリー・LLMO（Claude）・プロンプト拡張・AI ライティング・精密分析の AI 分析",
    ...
    usage: "精密分析の AI 分析 1 回 = Opus 5 で数万トークン（事実シートの量で変わる。数十〜数百円）。MEO の AI 総評 1 回 ≈ 5 円。FAQ 生成・判定・分類は Haiku 4.5（`LLM_FAST_MODEL`）",
    links: [
  ...
  openai: {
    ...
    description: "LLMO モニタリングで ChatGPT を対象にする。精密分析のセカンドオピニオン",
    ...
    usage: "LLMO の ChatGPT 列 = 1 質問 1 回（Web 検索つき）。精密分析のセカンドオピニオン 1 回 = 1 回（事実シート + Claude の分析を渡すので入力が多い）",
    links: [
  ...
  serpapi: {
    ...
    envVars: ["SERPAPI_KEY"],
    description: "順位計測・AI Overviews の引用チェック・ページ診断の上位 10 件取得・AIO 頻出トピック・精密分析の検索順位と site: 件数",
    ...
    usage: "精密分析 1 回 = 最大 7 検索（対策キーワード 5 + site: + ブランド名）。順位計測 = キーワード数 × 実行回数。ページ診断・AIO 頻出トピック 1 回 = 1 検索",
    links: [
  ...
  pagespeed: {
    ...
    usage: "精密分析 1 回 = PageSpeed 6 回（トップ + 主要 5 ページ、モバイル）+ CrUX 8 回（URL 6 + Origin + 推移）。ページ最適化レポート 1 回 = 1 回",
    links: [
  ...
  ahrefs: {
    ...
    usage: "精密分析 1 回 = 自社 + 競合 2 件で最大 3 回。同じドメインは 24 時間キャッシュ",
    links: [
  ...
  openpagerank: {
    ...
    description: "精密分析のドメインパワー。外部からの被リンクを見た 0〜10 の評価（無料。Ahrefs の DR が取れていればそちらを優先し、無ければこれで採点）",
    pricing: "無料",
    limits: "1 日 1,000 リクエスト、1 リクエストに 100 ドメインまで（無料枠）。超えると 429",
    usage: "精密分析 1 回 = 1 リクエスト（自社 + 競合をまとめて）。24 時間キャッシュ",
    links: [
  ...
  ga4: {
    ...
    usage: "生成 AI 流入分析・サイトレポートを 1 回開く = 数リクエスト。精密分析（連携済みなら）= 2〜3 リクエスト",
    links: [
  ...
  supabase: {
    ...
    description: "Google マップ・店舗情報（MEO）の登録店舗と診断報告書の履歴、口コミ支援、基本情報掲載、精密分析の実行記録（`analysis_runs`）",
    pricing: "Free プラン = $0（DB 500 MB、ファイル 1 GB、API リクエスト無制限、プロジェクト 2 つまで）。Pro = $25 / 月 / プロジェクト（DB 8 GB など。超過は従量）",
    limits: "無料プロジェクトは 1 週間アクセスが無いと一時停止（ダッシュボードで再開。週次 Cron が動いていれば止まらない）。DB 500 MB を超えると書き込みが止まる。精密分析の `audit` 列は 1 行で 1 MB 近くなるので、履歴が数百件になったら Pro か古い行の削除を検討",
