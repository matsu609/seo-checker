...
/**
 * どこまでをこのサービスがやるか（利用者の決定 2026-09-22）。柱ごとに線が違うので、
 * 画面の文言はここを引いて書く（同じ説明を各ページに書き写さない）。
 *
 *   SEO・AIO … 事実の提示と改善案の提示まで。ホームページへの反映はしない（できない）
 *   MEO      … 反映まで。Google ビジネス プロフィールは API で書き込めるので、
 *              口コミへの返信と投稿はこの画面から送る
 */
export const SCOPE_NOTE = {
  seo: "このツールはホームページを書き換えません。事実（いまどうなっているか）と、そのまま貼れる改善案までをお渡しします。反映はお客様・運用者の作業です。",
  meo: "Google ビジネス プロフィールを接続していれば、返信・投稿はこの画面から反映まで行えます。",
} as const;
...
export const FEATURE_CATEGORIES: readonly (FeatureCategory & { id: FeaturePillarId })[] = [
  { id: "seo", label: "SEO", description: "お持ちのホームページの状態を調べ、直すべき内容まで作って渡す。診断・改修案・FAQ・順位（反映はお客様・運用者）" },
  { id: "meo", label: "MEO", description: "Google マップ・ビジネス プロフィールの改善。口コミへの返信と投稿は、この画面から反映まで行える" },
...
const DIAGNOSIS: readonly Feature[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "トピックの出現割合・傾向・優先度の表",
      "自社ページに不足しているトピックの抽出（ページ改善・FAQ 提案へ引き継ぎ）",
    ],
    featureIds: ["A5"],
    icon: "topics",
    status: "beta",
  ...
  },
  {
    ...
    shortLabel: "ページ改善",
    description:
      "1 ページをよくするための画面です。対策キーワードを入れて 1 回実行すると、Google の上位 10 件と自社ページを実際に取得して比べ（事実）、その差を根拠にした改修案（そのまま貼れる形）までを同じ画面に出します。",
    details: [
      "上位 10 件と自社ページをダウンロードして測り、文字数・見出し・画像・リンクの差を出す（事実）",
      "その差と検索意図を根拠に、改修案を最大 5 件（before → after・なぜ直すか・期待できること・優先度）",
      "細かい所見（上位 10 件の一覧・技術的な指摘・追加すべき内容）は「詳しく見る」に畳んである",
      "対象ページは空欄なら、検索結果の中で自社ドメインの最上位ページを自動で選ぶ",
      "比較（事実）はライト、改修案の作成はスタンダード以上",
    ],
    featureIds: ["A4", "A2", "D2"],
    icon: "pen",
    status: "beta",
    ...
    group: "improve",
    category: "seo",
    // 2026-09-19 にページ診断（ライト）と HP 改修提案（スタンダード）を 1 画面 2 タブに統合し、
...
const MEASURE: readonly Feature[] = [
  {
    ...
    details: [
      "画面の先頭に順位の推移の折れ線（手動の計測と毎週の自動計測を同じ線に。上が 1 位）",
      "計測がまだのときは、登録済みのキーワードで「これからこう見える」を破線で描く（実測ではないと明記）",
  ...
  },
  {
    ...
    description:
      "ChatGPT / Gemini / Claude / Perplexity / AI Overviews / AI モード で、自社が「引用」「参照」される割合を毎週はかり、競合と並べます。「業界の地図」では、その話題の AI 回答で引用が多いサイトの順位表も引けます。1 回の結果で語らず、4 週ローリングと信頼区間で「言えることだけ」を出します。",
    details: [
 ...
 * AI が文章を作るもの。
 *
 * **SEO / AIO では「事実の提示」と「改善案の提示」までしかしない**（利用者の決定 2026-09-22）。
 * ホームページを直すのはお客様・運用者の仕事なので、貼る内容までを作って渡す。
 * AI ライティング・エディター（原稿そのものを書く機能）は、この線を越えるので同日に引退させた
 * （`/tools/writing` は「ページ改善」へ転送。コードは削除済み）。
 * MEO だけは反映まで行う（Google への口コミ返信・投稿はこの画面から送れる）。
 */
const GENERATE: readonly Feature[] = [
  {
    id: "faq",
    path: "/tools/faq",
    label: "FAQ 提案（AI に読み取らせる想定問答）",
    shortLabel: "FAQ 提案",
    description:
      "AI 検索（ChatGPT・Gemini・AI Overviews）は、質問と答えが対になっている文章をそのまま引用します。いまのページに FAQ が入っているかを確かめ、ページに書かれている事実だけを根拠に「入れるべき FAQ」を提案し、そのまま貼れる HTML と構造化データ（FAQPage）にして渡します。貼る作業はお客様・運用者が行います（このツールはホームページを書き換えません）。",
    details: [
      "いまの状態を確かめる: FAQPage の構造化データの有無、画面に見えている FAQ の数、構造化データと本文の食い違い（AI が読むのは両方そろっているとき）",
      "入れるべき FAQ の提案: 質問・回答案・なぜ必要か・優先度。回答はページ本文とお客様カルテに書かれている事実だけを根拠にする",
      "根拠が無い質問は「要確認」として、答えを作らずにお客様に確認していただく（AI に事実を作らせない）",
      "採用した分だけで JSON-LD と HTML を作成。構造化データと画面の文言は必ず一致させる（Google のガイドラインで、見えていない FAQ の構造化データは違反）",
    ],
    featureIds: [],
    icon: "prompt",
    status: "beta",
...
const SETTINGS: readonly Feature[] = [
  ...
  },
  {
    ...
    shortLabel: "お客様カルテ",
    description:
      "お店の強み・来てほしいお客様・よく聞かれる質問などを一度だけ教えていただくと、AI が書く文章（ページ改善の改修案・FAQ 案・口コミへの返信・Google マップの総評）が「どこにでもある一般論」から「お店の言葉」に変わります。業種に合わせた設問が出ます。ご要望の欄は運営者が必ず目を通し、次に作る機能を決める材料にします。",
    details: [
