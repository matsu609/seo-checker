 ...
 *
 * 順番は カード登録 → ホームページの登録 → 計測タグの設置 → 店舗登録 → ツール
 * （利用者の決定 2026-09-13、ホームページの登録を先頭に追加 2026-09-16）。
 * ホームページの URL はここで 1 回登録すれば、以後どのタブでも入力を求めない。
 * 先にカードを登録してもらうのは、無料期間中もすべての機能が開くため。先にタグを
 * 貼っても有料ツールが鍵のままだと数値が見られず、無駄足になる。
 *
...
export const ONBOARDING_STEPS: readonly OnboardingStep[] = [
  ...
  {
    n: 2,
    title: "計測タグをホームページに貼る",
    body: "アクセス解析の画面で発行される 1 行のタグを、ホームページの全ページ（テンプレートの </head> の直前）に貼ってください。Google アナリティクスの設定は不要です。貼った日から、訪問者数・流入元（検索 / 生成 AI / SNS）・電話やメールのタップが見られるようになります。検索の状況は、タグが無くても「検索パフォーマンス（推定）」でその日から見られます。",
