...
describe("メタデータからの読み取り", () => {
  it("public（直した値）が unsafe（登録時の値）より優先。無ければ null", () => {
    ...
    expect(leadFromMetadata({ [LEAD_KEY]: LEAD }, null)).toEqual(LEAD);
    expect(leadFromMetadata(null, { [LEAD_KEY]: LEAD })).toEqual(LEAD);
    expect(leadFromMetadata({ [LEAD_KEY]: { ...LEAD, company: "直した会社名" } }, { [LEAD_KEY]: LEAD })?.company).toBe("直した会社名");
    // 壊れた値は無視して次を見る
    expect(leadFromMetadata({ [LEAD_KEY]: { contactName: "だけ" } }, { [LEAD_KEY]: LEAD })).toEqual(LEAD);
  });
});
