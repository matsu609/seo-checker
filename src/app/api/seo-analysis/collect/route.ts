/**
 * POST /api/seo-analysis/collect — パワーアップ分析の「収集」。
 *
...
export async function POST(request: NextRequest) {
  ...
  if (!isSupabaseConfigured()) {
    return Response.json({ error: "パワーアップ分析には SUPABASE_URL と SUPABASE_SERVICE_ROLE_KEY の設定が必要です", code: "not_configured" }, { status: 503 });
  }
  if (!isAnthropicEnabled()) {
    return Response.json({ error: "パワーアップ分析には ANTHROPIC_API_KEY の設定が必要です", code: "not_configured" }, { status: 503 });
  }
