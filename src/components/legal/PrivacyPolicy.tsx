...
const PROMISES = [
  ...
  "Google Search Console・Google アナリティクスには接続しません。Google アカウントの連携を求めるのは、口コミ返信を使う場合のビジネス プロフィールだけです。",
];
...
const SECTIONS: Section[] = [
  ...
  {
    title: "第 2 条（取得する情報）",
    summary: "取得するのは、ログインに必要な情報、あなたが本サービスに入力した情報、そして技術的なアクセス記録です。",
    table: {
      head: ["種類", "内容", "保管場所"],
      rows: [
        ...
        { cells: ["Google 連携（口コミ返信）", "口コミ返信のために接続を許可した Google アカウント（ビジネス プロフィール）。アクセス権限（トークン）は認証基盤が保管します。Search Console・Google アナリティクスには接続しません", "Clerk"] },
        { cells: ["利用者が入力した情報", "診断対象の URL、キーワード、店舗名、プロジェクトと競合の登録、文章、アップロードした PDF 等", "主にお使いのブラウザ（localStorage）。診断のたびにサーバーへ送信され、処理後は保持しません。Google マップ・店舗情報（MEO）で登録した店舗（自社・競合の店名と Google の店舗 ID）だけはデータベース（Supabase）に保存します"] },
        { cells: ["診断結果", "採点、評価、提案、生成した文章。Google マップ・店舗情報（MEO）の診断報告書は、登録した店舗について毎週自動で作成し、履歴として保管します（Google マップ上の公開情報と採点、AI 総評を含みます）", "保存しない。MEO の診断報告書だけデータベース（Supabase）"] },
  ...
  },
  {
    ...
    table: {
      head: ["事業者", "役割", "渡る情報"],
      rows: [
        { cells: ["Clerk（米国）", "ログイン、ユーザー情報と Google 連携（口コミ返信）の権限の保管、契約管理", "アカウント情報、Google のアクセス権限、プラン情報"] },
        { cells: ["Vercel（米国）", "本サービスのホスティング、アクセスログ", "技術情報、処理中の入力情報"] },
        { cells: ["Google（米国）", "Google マップ / ビジネス プロフィール / PageSpeed Insights の API、ログイン", "診断対象の URL・店舗名、口コミ返信を許可した場合はその店舗の口コミと返信文"] },
        { cells: ["Anthropic（米国）", "生成 AI による総評・提案・文章の作成", "診断対象ページの本文や集計結果、店舗の公開情報と口コミ、利用者が入力したキーワード・文章"] },
        { cells: ["SerpApi（米国）", "検索結果の取得", "検索キーワード"] },
        { cells: ["Stripe / Clerk Billing（米国）", "有料プランの決済", "決済に必要な情報（運営者は取得しません）"] },
        { cells: ["Supabase（東京リージョン）", "MEO で登録した店舗と診断報告書の保管", "登録した店舗（店名と Google の店舗 ID）、毎週作成される診断報告書、その利用者のアカウント ID"] },
        { cells: ["DataForSEO（エストニア）", "検索パフォーマンス（推定）と AI 検索モニタリングのデータ取得", "診断対象のドメイン、登録したキーワード・プロンプト"] },
      ],
    },
    after: [
  ...
  },
  {
    ...
    summary: "Google Search Console・Google アナリティクスには接続しません。Google 連携で扱うのは、口コミ返信のために許可したビジネス プロフィールのデータだけで、本サービスの画面に表示し、利用者が書いた返信を投稿するためにしか使いません。",
    paragraphs: [
      "本サービスは Google Search Console および Google アナリティクスのデータを取得しません。検索の状況は連携を要しない推定値（検索パフォーマンス（推定））で扱います。",
      "口コミ返信の機能を使う場合に限り、利用者が明示的に許可したうえで Google アカウントに接続し、Google ビジネス プロフィールの口コミの取得と返信の投稿に必要な権限を要求します。それ以外の権限は要求しません。",
      "Google から取得したデータは、利用者本人に本サービスの画面（口コミへの返信、Google マップ・店舗情報など）で表示するためにのみ利用します。他の利用者に見せたり、広告に利用したり、本サービスの機能改善以外の目的で第三者に渡したりすることはありません。",
      "この権限で本サービスが行う操作は、(1) 利用者が接続したビジネス プロフィールのアカウントと店舗の一覧の取得、(2) その店舗の口コミと既存の返信の取得、(3) 利用者が画面で確認して「投稿する」を押した返信の投稿・更新・削除、の 3 つに限ります。返信を利用者の操作なしに自動で投稿することはなく、店舗情報（店名・住所・営業時間など）の書き換えも行いません。",
      "取得した口コミと返信は、画面に表示するたびに Google から取得し、運営者のサーバーやデータベースには保存しません。",
      "本サービスによる Google API から受け取った情報の使用および他のアプリへの転送は、限定的な使用（Limited Use）の要件を含む Google API サービスのユーザーデータに関するポリシーに準拠します。",
      "運営者は、Google から取得したデータを、汎用的な人工知能（AI）・機械学習モデルの開発・改善・学習には使用しません。",
      "AI が返信案を作る機能では、利用者本人に表示する下書きを作成する目的に限り、利用者が「AI で返信案を作る」を押した口コミの本文と評価を、運営者が契約する AI 事業者の API に送信します。送信した内容がモデルの学習に使われない設定・契約のもとで利用します（第 6 条）。作成された返信案は下書きであり、利用者が内容を確認・編集したうえで投稿の操作を行わない限り、Google には送られません。",
      "連携はいつでも解除できます。本サービスのアカウント設定から Google の接続を外すか、Google アカウントの「セキュリティ」→「サードパーティ製のアプリとサービス」から本サービスのアクセス権を削除してください。解除した時点で、本サービスが保管していたアクセス権限（トークン）は削除され、新たにデータを取得できなくなります。以前の連携で Search Console・Google アナリティクスの権限を付与していた場合も、本サービスはそれらを読み取りません。",
    ],
    links: [
  ...
  },
  {
    ...
    paragraphs: [
      "生成 AI を使う機能（総評、ページ改善の改修案、FAQ 生成・FAQ 提案、口コミの返信案など）では、診断対象ページの本文や集計結果、店舗の公開情報と口コミ、利用者が入力したキーワード・文章を、運営者が契約する AI 事業者の API に送信します。",
      ...
      "運営者は、送信した内容がモデルの学習に利用されない設定・契約のもとで、各 AI 事業者の API を利用します。とくに Google アカウントから取得したデータは、汎用的な AI・機械学習モデルの開発・改善・学習に使用しません。各 AI 事業者における保持期間・取り扱いは、各事業者の規約と方針に従います。",
    ],
  },
  {
    ...
    paragraphs: [
      ...
      "プロジェクト・競合の登録、画面の設定などは、お使いのブラウザの保存領域（localStorage）に保存されます。これらは運営者のサーバーには送られず、ブラウザのデータを消去すると失われます。設定画面から JSON 形式で書き出し・読み込みができます。",
    ],
  },
  {
    title: "第 8 条（保存期間と削除）",
    summary: "アカウントを消せば、預かっている情報も消えます。診断結果は、MEO の履歴以外は保存していません。",
    paragraphs: [
      "アカウント情報、Google 連携の権限と設定、プラン情報は、アカウントが削除されるまで保管します。Google 連携の権限（トークン）は、利用者が連携を解除した時点でも削除します。アカウントを削除すると、認証基盤上の情報は削除され、復元できません。",
      "診断のために送信された入力情報と診断結果は、処理のために一時的に扱うだけで、運営者のサーバーには保存しません（一部は応答を速くするため数分〜数時間の一時的なキャッシュに置かれることがあります）。MEO で登録した店舗と、その診断報告書の履歴、口コミ支援のアンケート・QR コード・回答は、画面からいつでも削除でき、アカウントの削除時にあわせて削除します。店舗の登録を外しても、その店舗の診断報告書の履歴は残りますが、履歴画面から個別に削除できます。",
  ...
  {
    title: "第 12 条（店舗のアンケートに回答する方の情報）",
    summary: "店内の QR コードから開くアンケートの回答は、その店舗（本サービスの利用者）に届きます。運営者はその保管を担うだけで、回答を別の目的には使いません。",
    paragraphs: [
      "本サービスには、利用者（店舗）が来店客向けのアンケートを作り、店内の QR コードから回答を受け取る機能があります。アンケートに回答する方は本サービスの利用者ではなく、回答時にアカウント登録やログインは求めません。",
      "回答（評価、選択、自由記述）、回答をもとに生成した口コミの下書きと回答者が編集した本文、「お店に直接伝える」で送られた内容と任意で入力された連絡先、回答の日時と QR コードの種別は、その店舗にのみ表示されます。運営者は、これらをサービスの提供（保管・表示・障害対応）のためにのみ取り扱い、店舗以外の第三者に提供せず、広告や営業に利用しません。回答をもとに口コミの下書きを作る際は、回答の文章を生成 AI（第 6 条）に送信します。氏名や連絡先は、回答者が自ら入力しない限り取得しません。",
      "回答は、店舗が削除するか、店舗がアンケートを削除するか、店舗のアカウントが削除されるまで保管します。回答者本人が自分の回答の削除を希望する場合は、その店舗または第 13 条の問い合わせ先にご連絡ください。",
      "Google マップへの口コミの投稿は、回答者がご自身の判断で Google のサービス上で行うものです。運営者は投稿の内容や有無を取得せず、投稿は Google のポリシーに従います。",
    ],
  },
  {
    title: "第 13 条（改定）",
