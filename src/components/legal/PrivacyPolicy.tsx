...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    table: {
      ...
      rows: [
        ...
        { cells: ["診断結果", "採点、評価、提案、生成した文章。Google マップ・店舗情報（MEO）の診断報告書は、利用者が「保存」を押したものだけを保管します（Google マップ上の公開情報と採点、AI 総評を含みます）", "保存しない。「保存」した MEO 診断報告書だけデータベース（Supabase）"] },
  ...
  },
  {
    ...
    table: {
      ...
      rows: [
        ...
        { cells: ["Supabase（東京リージョン）", "「保存」した MEO 診断報告書の保管", "利用者が保存を選んだ診断報告書と、その利用者のアカウント ID"] },
      ],
    },
    after: [
  ...
  {
    title: "第 8 条（保存期間と削除）",
    summary: "アカウントを消せば、預かっている情報も消えます。診断結果は、あなたが「保存」を押したもの以外は保存していません。",
    paragraphs: [
      "アカウント情報、Google 連携の権限と設定、プラン情報は、アカウントが削除されるまで保管します。アカウントを削除すると、認証基盤上の情報は削除され、復元できません。",
      "診断のために送信された入力情報と診断結果は、処理のために一時的に扱うだけで、運営者のサーバーには保存しません（一部は応答を速くするため数分〜数時間の一時的なキャッシュに置かれることがあります）。利用者が「保存」を押した MEO 診断報告書は、履歴画面からいつでも削除でき、アカウントの削除時にあわせて削除します。",
