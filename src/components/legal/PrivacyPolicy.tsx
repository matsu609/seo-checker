...
const PROMISES = [
  ...
  "ホームページに貼る計測タグは Cookie を使わず、訪問者の IP アドレスを保存しません。",
];
...
const SECTIONS: Section[] = [
  ...
  {
    title: "第 2 条（取得する情報）",
    summary: "取得するのは、ログインに必要な情報、あなたが本サービスに入力した情報、あなたのホームページに貼った計測タグが集める訪問記録、そして技術的なアクセス記録です。",
    table: {
      head: ["種類", "内容", "保管場所"],
      rows: [
        ...
        { cells: ["ホームページの訪問記録（計測タグ）", "利用者が自分のホームページに貼った計測タグから届く、閲覧したページのパス、参照元のサイト名、UTM パラメータ、端末の種別（スマホ / パソコン）、滞在時間とスクロール量、電話・メール・外部リンクのタップとフォーム送信の有無。訪問者の IP アドレスとブラウザ情報は保存せず、それらから作る日替わりのハッシュ（翌日には別の値になる識別子）だけを保存します", "Supabase（東京リージョン）"] },
  ...
  },
  {
    ...
    table: {
      ...
      rows: [
        ...
        { cells: ["Supabase（東京リージョン）", "MEO で登録した店舗と診断報告書、計測タグの訪問記録の保管", "登録した店舗（店名と Google の店舗 ID）、毎週作成される診断報告書、計測タグの訪問記録、その利用者のアカウント ID"] },
  ...
  },
  {
    ...
    paragraphs: [
      "本サービスは Google Search Console および Google アナリティクスのデータを取得しません。検索の状況は連携を要しない推定値（検索パフォーマンス（推定））で、ホームページ内の行動は利用者が貼る計測タグ（第 2 条・第 11 条）で扱います。",
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "利用者が自分のホームページに貼る計測タグは、そのホームページの訪問者のブラウザに Cookie を置かず、ブラウザの保存領域も使いません（第 11 条）。",
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "計測タグの訪問記録は、届いた日から 400 日で自動的に削除します。それより前の削除は、利用者からの申し出（第 13 条）により行います。",
  ...
  {
    title: "第 11 条（利用者が入力する第三者の情報と、利用者のホームページを訪れる方の情報）",
    summary: "競合サイトや口コミなど、他人の情報を扱う機能があります。また、計測タグは利用者のホームページの訪問者の記録を、その利用者のためだけに預かります。その扱いは、法令と各サービスのルールの範囲で、利用者の責任で行ってください。",
    paragraphs: [
      "本サービスには、利用者が自分のホームページに 1 行の計測タグを貼り、そのホームページの訪問状況（第 2 条の「ホームページの訪問記録」）を見る機能があります。訪問者は本サービスの利用者ではなく、訪問時にアカウント登録やログインは求めません。計測タグは Cookie もブラウザの保存領域も使わず、訪問者の IP アドレスとブラウザ情報を保存しません。訪問者を区別するための識別子は日替わりで変わるため、日をまたいで同じ人を追跡することはできません。",
      "訪問記録は、そのホームページの利用者にのみ表示し、運営者は障害調査と不正利用の防止以外の目的で閲覧しません。他の利用者に見せたり、広告に利用したり、第三者に提供したりすることはありません。計測タグの設置と、それに伴う自社サイトの訪問者への説明（自社サイトのプライバシーポリシーへの記載など）は、利用者の責任で行ってください。",
