...
const PROMISES = [
  ...
  "Google アカウントの連携を求めるのは、口コミ返信を使う場合のビジネス プロフィールと、Google サーチコンソール連携を使う場合の Search Console（読み取り専用）だけです。Google アナリティクスには接続しません。",
];
...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    table: {
      ...
      rows: [
        ...
        { cells: ["Google 連携（口コミ返信・サーチコンソール連携）", "口コミ返信またはサーチコンソール連携のために接続を許可した Google アカウント（ビジネス プロフィール、Search Console の読み取り）と、サーチコンソール連携で選んだサイトの URL。アクセス権限（トークン）は認証基盤が保管します。Google アナリティクスには接続しません", "Clerk"] },
  ...
  },
  {
    ...
    summary: "Google 連携で扱うのは、口コミ返信のために許可したビジネス プロフィールのデータと、サーチコンソール連携のために許可した Search Console のデータ（読み取りのみ）だけです。本サービスの画面に表示し、利用者が書いた返信を投稿するためにしか使いません。Google アナリティクスには接続しません。",
    paragraphs: [
      "本サービスは Google アナリティクスのデータを取得しません。",
      "口コミ返信の機能を使う場合に限り、利用者が明示的に許可したうえで Google アカウントに接続し、Google ビジネス プロフィールの口コミの取得と返信の投稿に必要な権限を要求します。",
      "Google サーチコンソール連携の機能を使う場合に限り、利用者が明示的に許可したうえで、Search Console の読み取り専用の権限を要求します。この権限で行う操作は、(1) 利用者のアカウントで見られるサイトの一覧の取得、(2) 利用者が選んだサイトの検索パフォーマンス（クリック数・表示回数・CTR・掲載順位、検索キーワード別・ページ別・日別）の取得、の 2 つに限ります。Search Console の設定の変更は行いません。取得したデータは画面に表示するために取得し、運営者のデータベースには保存しません（同じ内容を短時間に繰り返し取得しないよう、サーバーのメモリに最長 10 分間だけ一時的に保持します）。",
      "上記以外の権限は要求しません。",
      ...
      "連携はいつでも解除できます。本サービスのアカウント設定から Google の接続を外すか、Google アカウントの「セキュリティ」→「サードパーティ製のアプリとサービス」から本サービスのアクセス権を削除してください。解除した時点で、本サービスが保管していたアクセス権限（トークン）は削除され、新たにデータを取得できなくなります。以前の連携で Google アナリティクスの権限を付与していた場合も、本サービスはそれを読み取りません。",
    ],
    links: [
