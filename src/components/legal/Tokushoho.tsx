...
export const TOKUSHOHO_ROWS: Row[] = [
  ...
  {
    label: "販売価格",
    value: [...priceRows, "クイック診断（アカウント不要）: 0 円", "割引コードをお持ちの場合は、料金プランの画面で入力すると割引後の金額で決済されます（スタンダードのみ）。コードをお持ちでない場合は上記の価格です。コードの発行条件はお問い合わせください。"],
  },
    ...
    value:
      trial > 0
        ...
        : [
            ...
            "割引コードをお使いの場合は、コードの条件に応じた金額（0 円となる場合を含みます）で毎月の請求が立ちます。コードの条件にお申し込みから 30 日間の無料期間が含まれる場合は、お申し込み時はクレジットカードのご登録のみ行い、無料期間の終了日に初回の請求が立ち、以降は毎月同じ日に自動で決済されます。無料期間中に解約された場合、料金は発生しません。",
          ],
  },
