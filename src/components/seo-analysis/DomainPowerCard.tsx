...
export function DomainPowerCard({ domain }: { domain: DomainPowerResult }) {
  ...
  return (
    <Card
      title="ドメインパワー（推定）"
      description="サイト全体の地力を 0〜100 で表した推定値です。Ahrefs の DR や Moz の DA のような有料の被リンク指標とは別物で、無料で取れる 8 つの指標（外部リンクの評価・ドメインの年数・インデックス数・順位・実ユーザーの規模・規模・信頼）を配点して足し合わせています。取得できなかった指標は分母から外しています。"
      printCard
    ...
    >
      ...
      {domain.peers.length > 0 && (
        <div className="mt-5">
          ...
          <p className="mb-2 text-[11px] text-muted">競合サイトはクロールしていないため、外に公開されている 2 指標だけで比べています。</p>
          <table className="w-full border-collapse text-[12px]">
            <thead>
              <tr className="border-b border-line text-left text-[11px] text-muted">
                <th className="py-1.5 pr-2 font-normal">ドメイン</th>
                <th className="py-1.5 pr-2 text-right font-normal">外部リンクの評価（0〜10）</th>
