...
describe("日付の目盛りとイメージの値", () => {
  it("YYYY-MM-DD を M/D にする", () => {
    expect(dayLabel("2026-09-01")).toBe("9/1");
    expect(dayLabel("x")).toBe("x");
  });

  it("イメージは毎回同じ形（乱数を使わない）で、0 以上", () => {
    expect(sampleDailyClicks()).toEqual(sampleDailyClicks());
    expect(sampleDailyClicks()).toHaveLength(28);
    expect(Math.min(...sampleDailyClicks())).toBeGreaterThanOrEqual(0);
  });
});
